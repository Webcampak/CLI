...
class videoObj(object):
    ...
    def __init__(self, log, fileVideoLog = None):
        ...
        self.schema = {
            ...
            , "properties": {
                "type":                     {"type": ["string", "null"], "description": "Type of the video instance, usually daily, custom or postprod"}
                , "sourceFiles":            {"type": ["number", "null"], "description": "Number of JPG files copied to the directory for processing"}
                ...
                , "formats": {
                    ...
                    , "items": {
                        "type": "object"
                        , "properties": {
                            "name":     {"type": "string", "description": "Name of the format (usually 1080p, 720p, 480p, custom)"}
    ...
    def writeVideoFile(self):
        ...
        if self.writeJsonFile(self.videoFile, self.getVideo()) == True:
            self.log.info("videoObj.writeVideoFile(): " + _("Successfully saved last video file to: %(videoFile)s") % {'videoFile': str(self.videoFile)})
            return True
        else:
    ...
    def archiveVideoFile(self):
        ...
        if self.archiveJsonFile(self.fileVideoLog, self.getVideo()) == True:
            self.log.info("videoObj.archiveVideoFile(): " + _("Successfully archived video file to: %(videoFile)s") % {'videoFile': str(self.fileVideoLog)})
            return True
        else:
    ...
    def loadJsonFile(self, jsonFile):
        ...
        if os.path.isfile(jsonFile):
            self.log.info("videoObj.loadJsonFile(): " + _("Load JSON file into memory: %(jsonFile)s") % {'jsonFile': jsonFile} )                    
