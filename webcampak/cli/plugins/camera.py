...
class ExamplePluginController(CementBaseController):
    class Meta:
        ...
        description = 'Commands directly associated with camera operation'
        ...
        # ``$ webcampak camera --help``
        arguments = [
            (
                ['-s', '--sourceid'],
                dict(
                    help='Source ID to camera from',
                    action='store',
                    )
            )
        ]
    ...
    @expose(help="Get camera status")
    def status(self):
        self.app.log.info("Geting camera status", __file__)
        if self.app.pargs.config_dir != None:
...
def load(app):
    ...
    hook.register('post_argument_parsing', camera_plugin_hook)
