...
class xferUtils(object):
    ...
    def __init__(self, log, config_dir):
        ...
        self.maxFilesPerThread = int(
            self.configGeneral.getConfig("cfgxfermaxfilesperthread")
        )
    ...
    def loadJsonFile(self, jsonFile):
        ...
        """
        self.log.debug("xferUtils.loadJsonFile(): Start")
        if os.path.isfile(jsonFile):
            try:
                ...
                self.log.error(
                    "xferUtils.loadJsonFile(): File appears corrupted: %(jsonFile)s "
                    % {"jsonFile": jsonFile}
                )
        return {}
    ...
    def writeJsonFile(self, jsonFile, jsonContent):
        ...
        """
        self.log.debug("xferUtils.writeJsonFile(): Start")
    ...
    def logToJson(self, jsonFile, jsonContent, msg):
        ...
        """
        self.log.debug("xferUtils.logToJson(): Start")
        self.log.info("xferUtils.logToJson(): Log Message: %(msg)s " % {"msg": msg})
        jsonIdx = len(jsonContent["logs"]) + 1
        jsonContent["logs"][jsonIdx] = {
            "date": self.getCurrentDateIso(),
            "message": msg,
        }
    ...
    def getThreadsUUID(self):
        ...
        Returns:
            Array: threads UUID
        """
        self.log.debug("xferUtils.getThreadsUUID(): Start")
        threads = []
        for currentFilename in [
            ...
            if os.path.isdir(self.dirXferThreads + currentThreadUUID):
                ...
                self.log.info(
                    "xferUtils.getThreadsUUID(): Thread file: %(currentFilename)s does not have a thread directory, deleting JSON file ..."
    ...
    def getThreadPid(self, threadUUID):
        ...
        """
        self.log.debug("xferUtils.getThreadPid(): Start")
    ...
    def setThreadPid(self, threadUUID, pid):
        ...
        Returns:
            None
        """
        self.log.debug("xferUtils.setThreadPid(): Start")
        threadJson = self.loadJsonFile(self.dirXferThreads + threadUUID + ".json")
        threadJson["pid"] = pid
    ...
    def setThreadLastJob(self, threadUUID, jobReport):
        ...
        Returns:
            None
        """
        self.log.debug("xferUtils.setThreadLastJob(): Start")
        threadJson = self.loadJsonFile(self.dirXferThreads + threadUUID + ".json")
        if len(jobReport) == 0:
            jobReport = None
    ...
    def isPidAlive(self, pid):
        ...
        """
        self.log.debug("xferUtils.isPidAlive(): Start")
        if pid == None:
            return False
        else:
            try:
                os.kill(pid, 0)
                self.log.info(
                    "xferUtils.isPidAlive(): Process is running: %(pid)s "
                    % {"pid": str(pid)}
                )
                return True
            except OSError:
                self.log.info(
                    "xferUtils.isPidAlive(): Process does not exist: %(pid)s "
                    % {"pid": str(pid)}
                )
                return False
    ...
    def killThreadByPid(self, pid):
        ...
        """
        self.log.debug("xferUtils.killThreadByPid(): Start")
        try:
            os.kill(pid, signal.SIGKILL)  # or signal.SIGKILL  SIGTERM
            self.log.info(
                "xferUtils.isPidAlive(): Process has been killed: %(pid)s "
                % {"pid": str(pid)}
            )
            return True
        except OSError:
            self.log.info(
                "xferUtils.isPidAlive(): Process did not exist: %(pid)s "
                % {"pid": str(pid)}
            )
            return False
    ...
    def isThreadRunning(self, threadUUID):
        ...
        Returns:
            Boolean: Thread running or not
        """
        self.log.debug("xferUtils.isThreadRunning(): Start")
        threadJson = self.loadJsonFile(self.dirXferThreads + threadUUID + ".json")
        if "pid" in threadJson:
    ...
    def countThreadsQueue(self, threadUUID):
        ...
        """
        self.log.debug("xferUtils.countThreadsQueue(): Start")
        threadsFilesCount = 0
        for currentFilename in [
            f
            for f in os.listdir(self.dirXferThreads + threadUUID + "/")
            if f.endswith(".json")
        ]:
    ...
    def getThreadFiles(self, threadUUID):
        ...
        """
        self.log.debug("xferUtils.getThreadFiles(): Start")
        allThreadFiles = []
        for currentFilename in [
            f
            for f in os.listdir(self.dirXferThreads + threadUUID + "/")
            ...
            allThreadFiles.append(
                os.path.join(self.dirXferThreads + threadUUID + "/", currentFilename)
            )
        allThreadFiles.sort()
    ...
    def getFirstThreadFile(self, threadUUID):
        ...
        Returns:
            String: Filename
        """
        self.log.debug("xferUtils.getFirstThreadFile(): Start")
        allThreadFiles = []
        for currentFilename in [
            f
            for f in os.listdir(self.dirXferThreads + threadUUID + "/")
    ...
    def isThreadFull(self, threadUUID):
        ...
        """
        self.log.debug("xferUtils.isThreadFull(): Start")
    ...
    def areThreadsFull(self):
        ...
        Returns:
            Boolean: True of False depending if the thread is full or not
        """
        self.log.debug("xferUtils.areThreadsFull(): Start")
        threadsFull = True
    ...
    def checkThreadUUID(self, threadUUID):
        ...
        """
        self.log.debug("xferUtils.checkThreadUUID(): Start")
    ...
    def getAllQueuedFiles(self):
        ...
        Returns:
            Boolean: True of False depending if the thread is valid or not
        """
        self.log.debug("xferUtils.getAllQueuedFiles(): Start")
        allQueuedFiles = []
