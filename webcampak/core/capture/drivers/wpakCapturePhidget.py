...
class capturePhidget(object):
    ...
    # Return: Nothing
    def capture(self):
        self.log.info("capturePhidget.capture(): Start capturing sensor values")
        # Get a list of all sensors for the source
        allSensors = self.getConfigSensors()
        self.log.info(
            "capturePhidget.capture(): Start %(allSensors)s"
            % {"allSensors": str(allSensors)}
        )
        capturedSensors = {}
        if len(allSensors) > 0:
            ...
            if phidgetsClass.attachPhidgetKit() == True:
                for currentSensor in allSensors:
                    ...
                    if sensorType != "" and sensorPort != "":
                        ...
                        sensorTypeConfig = self.configGeneral.getConfig(
                            "cfgphidgetsensortype" + str(sensorType)
                        )
                        sensorTypeName = sensorTypeConfig[0]
                        sensorTypeFormula = sensorTypeConfig[3]
                        self.log.info(
                            "capturePhidget.capture(): Capturing sensor: %(sensorTypeName)s from port: %(sensorPort)s"
                            % {
                                "sensorTypeName": str(sensorTypeName),
                                "sensorPort": str(sensorPort),
                            }
                        )
                        SensorValue = int(phidgetsClass.getSensorRawValue(sensorPort))
                        sensorCalculatedValue = round(eval(sensorTypeFormula), 1)
                        self.log.info(
                            "capturePhidget.capture(): Captured value, RAW: %(SensorValue)s Interpreted: %(sensorCalculatedValue)s"
                            % {
    ...
    def getConfigSensors(self):
        self.log.info("capturePhidget.getConfigSensors(): Start")
        fullSourceConfig = self.configSource.getFullConfig()
        sensorsCfg = []
        for configIdx in fullSourceConfig:
            if (
                "cfgphidgetsensor" in configIdx
                and configIdx != "cfgphidgetsensornb"
                and "cfgphidgetsensorinsert" not in configIdx
                and "cfgphidgetsensorsgraph" not in configIdx
            ):
                if (
                    fullSourceConfig[configIdx][0] != ""
                    and fullSourceConfig[configIdx][1] != ""
                ):
