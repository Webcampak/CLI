...
class captureRtsp(object):
    ...
    # Return: Nothing
    def capture(self):
        self.log.debug("captureRtsp.capture(): Start")
        ...
        self.log.info(
            "captureRtsp.capture(): Starting capture process, URL: %(URL)s "
        ...
        self.log.info(
            "captureRtsp.capture(): Capture Command: %(captureCommand)s "
        ...
        if os.path.isfile(self.dirCurrentSourceTmp + self.captureFilename + ".jpg"):
            FileSourceSize = os.path.getsize(
                self.dirCurrentSourceTmp + self.captureFilename + ".jpg"
            )

        if FileSourceSize < int(self.configSource.getConfig("cfgcaptureminisize")):
            self.log.info(
                "captureRtsp.capture(): Retrying capture process, URL: %(URL)s "
                % {"URL": self.configSource.getConfig("cfgsourcewebfileurl")}
            )
            os.system(Command)
            if os.path.isfile(self.dirCurrentSourceTmp + self.captureFilename + ".jpg"):
        ...
        if FileSourceSize < int(self.configSource.getConfig("cfgcaptureminisize")):
            ...
            if self.captureUtils.verifyCapturedFile(
                ...
                self.log.error(
                    "captureRtsp.triggerCapture(): Failed to capture from Camera"
                )
                if os.path.isfile(
                    self.dirCurrentSourceTmp + self.captureFilename + ".jpg"
                ):
