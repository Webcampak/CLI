...
class captureWpak(object):
    ...
    # Return: Nothing
    def capture(self):
        self.log.info("captureWpak.capture(): Start")
        getFromSourceID = self.configSource.getConfig("cfgsourcewpakgetsourceid")
        if getFromSourceID == self.currentSourceId:
            self.log.error("captureWpak.capture(): A source cannot capture from itself")
            return False
        elif int(getFromSourceID) > 0:
            self.log.info(
                "captureWpak.capture(): Looking for JPG file into source %(getFromSourceID)s live directory"
            ...
            if os.path.isfile(dstSourceLiveDir + "last-capture.jpg"):
                ...
                self.lastCaptureDetails.setCaptureFile(
                    dstSourceLiveDir + "last-capture.json"
                )
                ...
                self.captureClass.setCaptureFilename(
                    self.captureClass.getCaptureTime().strftime("%Y%m%d%H%M%S")
                )
                self.captureFilename = self.captureClass.getCaptureFilename()

                if (
                    self.captureUtils.getArchivedSize(self.captureFilename, "jpg") == 0
                ):  # Only process the capture
                    self.currentCaptureDetails.setCaptureValue(
                        "captureDate", self.captureClass.getCaptureTime().isoformat()
                    )

                    self.log.info(
                        "captureWpak.capture(): Copying file last-capture.jpg  from %(sourceLiveDirectory)s to %(Cfgtmpdir)s"
                        % {
                    ...
                    shutil.copy(
                        dstSourceLiveDir + "last-capture.jpg",
                        self.dirCurrentSourceTmp + self.captureFilename + ".jpg",
                    )
                    if (
                        os.path.isfile(dstSourceLiveDir + "last-capture.raw")
                        and self.configSource.getConfig("cfgprocessraw") == "yes"
                    ):
                        self.log.info(
                            "captureWpak.capture(): Copying file last-capture.raw  from %(sourceLiveDirectory)s to %(Cfgtmpdir)s"
                            % {
                                "sourceLiveDirectory": dstSourceLiveDir,
                                "Cfgtmpdir": self.dirCurrentSourceTmp,
                            }
                        ...
                        shutil.copy(
                            dstSourceLiveDir + "last-capture.raw",
                            self.dirCurrentSourceTmp + self.captureFilename + ".raw",
                        )
                    else:
                        self.log.info(
                            "captureWpak.capture(): Raw processing is either disabled or last-capture.raw do not exist in %(sourceLiveDirectory)s live directory"
                            % {"sourceLiveDirectory": dstSourceLiveDir}
                        )
                    ...
                    if self.captureUtils.verifyCapturedFile(
                        ...
                        self.log.error(
                            "captureWpak.triggerCapture(): Failed to capture from Camera"
                        )
                        if os.path.isfile(
                            self.dirCurrentSourceTmp + self.captureFilename + ".jpg"
                        ):
                    ...
                    self.log.error(
                        "captureWpak.capture(): File already captured: %(captureFilename)s"
                        % {"captureFilename": str(self.captureFilename + ".jpg")}
                    )
                    return False
            else:
                self.log.info(
                    "captureWpak.capture(): Error: last-capture.jpg file is missing in %(sourceLiveDirectory)s directory"
                    % {"sourceLiveDirectory": dstSourceLiveDir}
                )
                return False
        else:
            self.log.info(
                "captureWpak.capture(): Source %(getFromSourceID)s is not a valid source"
                % {"getFromSourceID": str(getFromSourceID)}
            )
            return False
