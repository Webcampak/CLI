...
class captureGphoto(object):
    ...
    def scanPorts(self):
        self.log.debug("captureGphoto.scanPorts(): Start")
        gphotoScanPortsCommand = (
            self.configGeneral.getConfig("cfggphotodir")
        ...
        try:
            ...
            self.log.info("captureGphoto.scanPorts(): Gphoto busy")
            return None
    ...
    def getCameraOwner(self, usbport):
        self.log.debug("captureGphoto.getCameraOwner(): Start")
        CmdGphotoSearchOwner = (
            self.configGeneral.getConfig("cfggphotodir")
            + "gphoto2 --port="
            + usbport
            + " --get-config=/main/settings/ownername"
        )
    ...
    def cameraLookup(self):
        self.log.debug("captureGphoto.cameraLookup(): Start")
        if self.configGeneral.getConfig("cfggphotoports") == "yes":
            self.log.info(
                "captureGphoto.cameraLookup(): Capture previously failed, checking USB port of the camera"
            )
            ...
            if gphotoPorts != None:
                ...
                for line in gphotoPorts:
                    ...
                    self.log.info(
                        "captureGphoto.cameraLookup(): USB Port: %(USBPort)s"
                        % {"USBPort": resultusb.group(0)}
                    )
                    if (
                        self.configGeneral.getConfig("cfggphotoportscameras")
                        == "different"
                    ):
                        if self.configSource.getConfig(
                            "cfgsourcegphotocameramodel"
                        ) in resultcamera.group(0):
                            self.log.info(
                                "captureGphoto.cameraLookup(): Camera: %(USBCamera)s"
                                % {"USBCamera": resultcamera.group(0)}
                            )
                            self.log.info(
                                "captureGphoto.cameraLookup(): Camera in config file: %(USBCameraConfig)s"
                                % {
                                    "USBCameraConfig": self.configSource.getConfig(
                                        "cfgsourcegphotocameramodel"
                                    )
                                }
                            )
                            self.log.info(
                                "captureGphoto.cameraLookup(): USB Port configured: %(USBPortConfigured)s"
                                % {
                            ...
                            self.log.info(
                                "captureGphoto.cameraLookup(): USB Port detected: %(USBPordDetected)s"
                                % {"USBPordDetected": resultusb.group(0)}
                            )
                            try:
                                self.log.info(
                                    "captureGphoto.cameraLookup(): Modification of the configuration file"
                                )
                                self.configSource.setConfig(
                                    "cfgsourcegphotocameraportdetail",
                                    resultusb.group(0),
                                )
                                self.log.info(
                                    "captureGphoto.cameraLookup(): New USB Port configured: %(USBPortConfigured)s"
                                    % {
                                        "USBPortConfigured": self.configSource.getConfig(
                                            "cfgsourcegphotocameraportdetail"
                                        )
                                ...
                                self.log.info(
                                    "captureGphoto.cameraLookup(): Unable to modify the configuration file"
                                )
                        else:
                            self.log.info(
                                "captureGphoto.cameraLookup(): Unknown camera"
                            )
                    elif (
                        self.configGeneral.getConfig("cfggphotoportscameras")
                        == "identical"
                        ...
                        self.log.info(
                            "captureGphoto.cameraLookup(): Configured camera: Port: %(USBPortConfigured)s, Owner: %(OwnerConfigured)s"
                            % {
                                "USBPortConfigured": self.configSource.getConfig(
                                    "cfgsourcegphotocameraportdetail"
                                ),
                                "OwnerConfigured": self.configSource.getConfig(
                                    "cfgsourcegphotoowner"
                                ),
                            }
                        )
                        self.log.info(
                            "captureGphoto.cameraLookup(): Detected Camera: Port: %(USBPortConfigured)s, Owner: %(OwnerConfigured)s"
                            % {
                                "USBPortConfigured": resultusb.group(0),
                                "OwnerConfigured": CameraOwner,
                            }
                        )
                        if self.configSource.getConfig(
                            "cfgsourcegphotoowner"
                        ) == CameraOwner and self.configSource.getConfig(
                            "cfgsourcegphotocameraportdetail"
                        ) != resultusb.group(
                            0
                        ):
                            self.log.info(
                                "captureGphoto.cameraLookup(): Camera owner identified"
                            )
                            try:
                                self.log.info(
                                    "captureGphoto.cameraLookup(): Modification of the configuration file"
                ...
                self.log.info("captureGphoto.cameraLookup(): Call to Gphoto failed")

    def appendCameraPort(self):
        self.log.debug("captureGphoto.appendCameraPort(): Start")
        if (
            self.configSource.getConfig("cfgsourcegphotocameraportdetail")
            != "automatic"
            and self.configSource.getConfig("cfgsourcegphotocameraportdetail") != ""
            and self.configGeneral.getConfig("cfggphotoports") == "yes"
        ):
            GphotoPortCommand = "--port=" + self.configSource.getConfig(
                "cfgsourcegphotocameraportdetail"
            )
            self.log.info(
                "captureGphoto.capture(): Gphoto: Precision camera port: %(CameraPort)s"
                % {
                    "CameraPort": self.configSource.getConfig(
                        "cfgsourcegphotocameraportdetail"
                    )
                }
            )
    ...
    def appendDebugMode(self):
        self.log.debug("captureGphoto.appendDebugMode(): Start")
        if self.configSource.getConfig("cfgsourcedebug") == "yes":
            ...
            self.log.info("captureGphoto.capture(): Gphoto: Debug mode activation")
        else:
            GphotoDebugCommand = ""
        return GphotoDebugCommand

    def triggerCapture(self):
        self.log.debug("captureGphoto.triggerCapture(): Start")
        ...
        self.log.info("captureGphoto.triggerCapture(): Gphoto: Start Capture")
        if os.path.isfile(self.configGeneral.getConfig("cfggphotodir") + "gphoto2"):
            ...
            if os.path.isfile(self.dirCurrentSourceTmp + self.captureFilename + ".JPG"):
                self.log.info(
                    "captureGphoto.triggerCapture(): Gphoto: Renaming raw picture from .JPG to .jpg"
                )
                os.rename(
                    self.dirCurrentSourceTmp + self.captureFilename + ".JPG",
                    self.dirCurrentSourceTmp + self.captureFilename + ".jpg",
            ...
            if os.path.isfile(self.dirCurrentSourceTmp + self.captureFilename + ".CR2"):
                self.log.info(
                    "captureGphoto.triggerCapture(): Gphoto: Renaming raw picture from .CR2 to .raw"
                )
                os.rename(
                    self.dirCurrentSourceTmp + self.captureFilename + ".CR2",
            ...
            if os.path.isfile(self.dirCurrentSourceTmp + self.captureFilename + ".nef"):
                self.log.info(
                    "captureGphoto.triggerCapture(): Gphoto: Renaming raw picture from .nef to .raw"
                )
                os.rename(
                    self.dirCurrentSourceTmp + self.captureFilename + ".nef",
                    self.dirCurrentSourceTmp + self.captureFilename + ".raw",
                )

            if os.path.isfile(self.dirCurrentSourceTmp + self.captureFilename + ".nrw"):
                self.log.info(
                    "captureGphoto.triggerCapture(): Gphoto: Renaming raw picture from .nrw to .raw"
                )
                os.rename(
                    self.dirCurrentSourceTmp + self.captureFilename + ".nrw",
                    self.dirCurrentSourceTmp + self.captureFilename + ".raw",
                )
            ...
            if self.configSource.getConfig("cfgprocessraw") != "yes" and os.path.isfile(
                ...
                self.log.info(
                    "captureGphoto.triggerCapture(): Gphoto: Raw file not allowed, deleting file"
                )
            ...
            if not os.path.isfile(
                ...
                self.log.info(
                    "captureGphoto.triggerCapture(): Gphoto: No jpg file captured, converting raw file to jpg"
                )
                import shlex, subprocess
                ...
                output, errors = p.communicate()
                self.log.info(
                    "captureGphoto.triggerCapture(): Gphoto: Conversion to jpg completed"
                )
            ...
            if self.captureUtils.verifyCapturedFile(
                ...
                self.log.error(
                    "captureGphoto.triggerCapture(): Failed to capture from Camera"
                )
                if os.path.isfile(
                    self.dirCurrentSourceTmp + self.captureFilename + ".jpg"
                ):
    ...
    # Return: Nothing
    def capture(self):
        self.log.debug("captureGphoto.capture(): Start")
        self.log.info("captureGphoto.capture(): Initiating capture")
        capturedPicture = self.triggerCapture()
        if capturedPicture != False:
            self.log.info("captureGphoto.capture(): Capture successful")
            return capturedPicture
        self.log.info(
            "captureGphoto.capture(): Capture failed, total number of consecutive failures: %(error_count)s"
            % {"error_count": str(self.captureUtils.getCustomCounter("errorcount"))}
        )
        if (
            self.configSource.getConfig("cfgphidgetcameraactivate") == "yes"
            and self.captureUtils.getCustomCounter("errorcount") < 5
        ):
            ...
            self.log.info(
                "captureGphoto.capture(): Proceeding with a Phidget restart of the camera (powercycle)"
            )
