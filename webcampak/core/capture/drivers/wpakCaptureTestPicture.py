...
class captureTestPicture(object):
    ...
    def capture(self):
        self.log.info("captureTestPicture.capture(): Start picture acquisition")
        ...
        self.log.info(
            "captureTestPicture.capture(): Copy test file to temporary directory"
        )
        shutil.copy(
            self.dirResources + "watermark/sample-picture.jpg",
            self.dirCurrentSourceTmp + captureFilename + ".jpg",
        )
        self.log.info(
            "captureTestPicture.capture(): Temporary capture file: %(tempCaptureFile)s"
            % {"tempCaptureFile": self.dirCurrentSourceTmp + captureFilename + ".jpg"}
        )

        self.log.info(
            "captureTestPicture.capture(): Applying random blur effect (just to be able to see the difference from one picture to another)"
        )
        ...
        if self.captureUtils.verifyCapturedFile(
            ...
            self.log.error(
                "captureTestPicture.triggerCapture(): Failed to capture from Camera"
            )
