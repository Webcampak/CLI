...
class captureIPCam(object):
    ...
    def GetExifDate(self, picture):
        self.log.debug("captureIPCam.GetExifDate(): Start")
        try:
            ...
            if exifinfo != None:
                for tag, value in list(exifinfo.items()):
                    ...
                    if decoded == "DateTimeDigitized":
                        ...
                        # 2012:05:20 10:46:37
                        cfgnow = datetime(
                            *time.strptime(value, "%Y:%m:%d %H:%M:%S")[0:6]
                        )
                        return cfgnow
        except:
            return 0
    ...
    def processFile(self, filePath):
        self.log.info("captureIPCam.processFile(): " + "Start Capture")
        self.log.info(
            "captureIPCam.processFile(): "
            + "Processing: %(filePath)s, size %(fileSize)s"
            % {"filePath": filePath, "fileSize": os.path.getsize(filePath)}
        )
        fileName, fileExtension = os.path.splitext(os.path.basename(filePath))
        if (fileExtension == ".jpg" or fileExtension == ".JPG") and os.path.getsize(
            filePath
        ) > int(self.configSource.getConfig("cfgcaptureminisize")):
            self.log.info("captureIPCam.processFile(): " + "File is a picture")

            if self.configSource.getConfig("cfgsourcecamiptemplate") == "webcampak" or (
                self.configSource.getConfig("cfgsourcewpaktype") == "rec"
                and self.configSource.getConfig("cfgsourcetype") == "wpak"
            ):
                self.log.info(
                    "captureIPCam.processFile(): Determining picture date based on webcampak name template (YYYYMMDDHHMMSS.jpg)"
                )
                currentFileTime = self.timeUtils.getTimeFromFilename(
                    fileName, self.configSource, "YYYYMMDDHHMMSS"
                )
            elif (
                self.configSource.getConfig("cfgsourcecamiptemplate") == "harbortronics"
            ):
                self.log.info(
                    "captureIPCam.processFile(): "
                    + "Determining picture date based on harbortonics naming convention"
                )
                currentFileTime = self.timeUtils.getTimeFromFilename(
                    fileName, self.configSource, "YYYYMMDD_HHMMSS"
                )
            elif self.configSource.getConfig("cfgsourcecamiptemplate") == "tplink":
                # 192.168.0.18_01_20170103153300_TIMING - Regex: ([0-9]{14})+
                self.log.info(
                    "captureIPCam.processFile(): "
                    + "Determining picture date based on TPLINK naming convention"
                )
                regex = r"([0-9]{14})+"
                extractedDatetime = re.findall(regex, fileName)
                currentFileTime = self.timeUtils.getTimeFromFilename(
                    extractedDatetime[0], self.configSource, "YYYYMMDDHHMMSS"
                )
            elif self.configSource.getConfig("cfgsourcecamiptemplate") == "exif":
                self.log.info(
                    "captureIPCam.processFile(): "
                    + "Determining picture date based on EXIF details"
                )
                currentFileTime = self.timeUtils.getTimeFromExif(
                    filePath, self.configSource
                )
            elif self.configSource.getConfig("cfgsourcecamiptemplate") == "filedate":
                self.log.info(
                    "captureIPCam.processFile(): "
                    + "Determining picture date based on filesystem date"
                )
                currentFileTime = self.timeUtils.getTimeFromFiledate(
                    filePath, self.configSource
                )
            if currentFileTime == False:
                self.log.info(
                    "captureIPCam.processFile(): Unable to get date based on configured method , looking into EXIF details"
                )
                currentFileTime = self.timeUtils.getTimeFromExif(
                    filePath, self.configSource
                )
            if currentFileTime == False:
                self.log.info(
                    "captureIPCam.processFile(): Unable to get date based on configured method , looking into filesystem date"
                )
                currentFileTime = self.timeUtils.getTimeFromFiledate(
                    filePath, self.configSource
                )
            if currentFileTime == False:
                self.log.info(
                    "captureIPCam.processFile(): "
                    + "Unable to get date, cancelling processing"
                )
                return None

            self.currentCaptureDetails.setCaptureValue(
                "captureDate", currentFileTime.isoformat()
            )
            ...
            self.log.info(
                "captureIPCam.processFile(): "
                + "Moving file to root of /tmp directory for consistent processing"
            )
            shutil.move(
                filePath, self.dirCurrentSourceTmp + self.captureFilename + ".jpg"
            )
            if os.path.isfile(os.path.splitext(filePath)[0] + ".raw"):
                shutil.move(
                    os.path.splitext(filePath)[0] + ".raw",
            ...
            self.log.info(
                "captureIPCam.processFile(): Processing a sensor file: %(filePath)s"
            ...
            self.log.info(
                "captureIPCam.processFile(): "
                + "Sensor file copied to %(destinationSensorFilePath)s"
                % {"destinationSensorFilePath": str(destinationSensorFilePath)}
            )
        else:
            self.log.info(
                "captureIPCam.processFile(): %(filePath)s is not a picture or is too small"
                % {"filePath": os.path.basename(filePath)}
            )
            return None
    ...
    # Return: Nothing
    def capture(self):
        self.log.info("captureIPCam.capture(): Start Capture")
        self.log.info(
            "captureIPCam.capture(): "
            + "Entering the process, template: %(Template)s"
            % {"Template": self.configSource.getConfig("cfgsourcecamiptemplate")}
        )

        capturedFiles = []

        for firstLevelScanFile in sorted(
            os.listdir(self.dirCurrentSourceTmp), reverse=True
        ):
            self.log.info(
                "captureIPCam.capture(): "
                + "Processing: %(firstLevelScanFile)s"
                % {"firstLevelScanFile": firstLevelScanFile}
            )
            if (
                os.path.isdir(self.dirCurrentSourceTmp + firstLevelScanFile)
                and firstLevelScanFile[:2] == "20"
            ):
                self.log.info(
                    "captureIPCam.capture(): "
                    + "%(firstLevelScanFile)s is a directory, scanning"
                    % {"firstLevelScanFile": firstLevelScanFile}
                )
                for secondLevelScanFile in sorted(
                    os.listdir(self.dirCurrentSourceTmp + firstLevelScanFile),
                    reverse=True,
                ):
                    self.log.info(
                        "captureIPCam.capture(): "
                        + "Processing %(secondLevelScanFile)s"
                        % {
                            "secondLevelScanFile": firstLevelScanFile
                            + "/"
                            + secondLevelScanFile
                        }
                    )
                ...
                self.log.info(
                    "captureIPCam.capture(): "
                    + "%(firstLevelScanFile)s is a file"
                    % {"firstLevelScanFile": firstLevelScanFile}
                )
                currentProcessedPictures = self.processFile(
                    self.dirCurrentSourceTmp + firstLevelScanFile
                )
                if currentProcessedPictures != None:
                    capturedFiles.append(currentProcessedPictures)
