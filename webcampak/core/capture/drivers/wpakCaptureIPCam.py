...
class captureIPCam(object):
    ...
    def processFile(self, filePath):
        ...
        self.log.info("captureIPCam.processFile(): " + _("Processing: %(filePath)s, size %(fileSize)s") % {'filePath': filePath, 'fileSize': os.path.getsize(filePath)})
        fileName, fileExtension = os.path.splitext(os.path.basename(filePath))
        if fileExtension == '.jpg' and os.path.getsize(filePath) > int(self.configSource.getConfig('cfgcaptureminisize')):
            self.log.info("captureIPCam.processFile(): " + _("File is a picture"))
                                    
            if self.configSource.getConfig('cfgsourcecamiptemplate') == "webcampak" or (self.configSource.getConfig('cfgsourcewpaktype') == "rec" and self.configSource.getConfig('cfgsourcetype') == "wpak"):            
                self.log.info("captureIPCam.processFile(): " + _("Determining picture date based on webcampak name template (YYYYMMDDHHMMSS.jpg)"))     
            ...
            if currentFileTime == False:
                self.log.info("captureIPCam.processFile(): " + _("Unable to get date based on configured method , looking into EXIF details")) 
                currentFileTime = self.timeUtils.getTimeFromExif(filePath, self.configSource)                
            if currentFileTime == False:    
                self.log.info("captureIPCam.processFile(): " + _("Unable to get date based on configured method , looking into filesystem date")) 
                currentFileTime = self.timeUtils.getTimeFromFiledate(filePath, self.configSource)
            if currentFileTime == False:    
            ...
            self.log.info("captureIPCam.processFile(): " + _("Moving file to root of /tmp directory for consistent processing"))     
            shutil.move(filePath, self.dirCurrentSourceTmp + self.captureFilename + ".jpg")
            if os.path.isfile(os.path.splitext(filePath)[0] + ".raw"):
                shutil.move(os.path.splitext(filePath)[0] + ".raw", self.dirCurrentSourceTmp + self.captureFilename + ".raw")
                
            return self.dirCurrentSourceTmp + self.captureFilename + ".jpg"
        else:
            self.log.info(_("captureIPCam.processFile(): %(filePath)s is not a picture or is too small") % {'filePath': os.path.basename(filePath)})
            return None
    ...
    def capture(self):
        ...
        self.log.info("captureIPCam.capture(): " + _("Entering the process, template: %(Template)s") % {'Template': self.configSource.getConfig('cfgsourcecamiptemplate')} )
        
        capturedFiles = []
        
        for firstLevelScanFile in sorted(os.listdir(self.dirCurrentSourceTmp), reverse=True):
            self.log.info("captureIPCam.capture(): " + _("Processing: %(firstLevelScanFile)s") % {'firstLevelScanFile': firstLevelScanFile} )                
            if os.path.isdir(self.dirCurrentSourceTmp + firstLevelScanFile) and firstLevelScanFile[:2] == "20":
                self.log.info("captureIPCam.capture(): " + _("%(firstLevelScanFile)s is a directory, scanning") % {'firstLevelScanFile': firstLevelScanFile} )                                    
                for secondLevelScanFile in sorted(os.listdir(self.dirCurrentSourceTmp + firstLevelScanFile), reverse=True):
                    self.log.info("captureIPCam.capture(): " + _("Processing %(secondLevelScanFile)s") % {'secondLevelScanFile': firstLevelScanFile + "/" + secondLevelScanFile} )
                    currentProcessedPictures = self.processFile(self.dirCurrentSourceTmp + firstLevelScanFile + "/" + secondLevelScanFile)
                    if currentProcessedPictures != None:
                        capturedFiles.append(currentProcessedPictures)
            else:    
                self.log.info("captureIPCam.capture(): " + _("%(firstLevelScanFile)s is a file") % {'firstLevelScanFile': firstLevelScanFile} )
