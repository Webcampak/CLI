...
class captureEmails(object):
    ...
    def sendCaptureSuccess(self, captureFilename=None):
        ...
        Returns:
            None
        """
        self.log.debug("captureEmails.sendCaptureSuccess(): Start")
        ...
        if os.path.isfile(emailSuccessContent) == False:
            emailSuccessContent = (
                ...
                + "emailOnlineContent.txt"
            )
            emailSuccessSubject = (
                self.dirLocale
                + "en_US.utf8/"
                + self.dirLocaleMessage
                + "emailOnlineSubject.txt"
            )
        self.log.info(
            "capture.sendCaptureSuccess(): Using message subject file: %(emailSuccessSubject)s"
            % {"emailSuccessSubject": emailSuccessSubject}
        )
        self.log.info(
            "capture.sendCaptureSuccess(): Using message content file: %(emailSuccessContent)s"
        ...
        if os.path.isfile(emailSuccessContent) and os.path.isfile(emailSuccessSubject):
            ...
            emailSubject = emailSubject.replace(
                "#NBFAILURES#", str(self.captureUtils.getCustomCounter("errorcount"))
            )
            newEmail = Email(
                self.log,
            ...
            if (
                ...
                and int(self.configSource.getConfig("cfgemailsuccesspicturewidth")) > 0
            ):
                ...
                if os.path.isfile(
                    ...
                    newEmail.attachments.append(
                        {
                            "PATH": captureFilename,
                            "WIDTH": int(
                                self.configSource.getConfig(
                                    "cfgemailsuccesspicturewidth"
                                )
                            ),
    ...
    def sendCaptureError(self, sendEmailErrorCount, lastSuccessCapture):
        ...
        """
        self.log.info(
            "captureEmails.sendCaptureError(): Preparation of an email alert about the error"
        )
        ...
        if os.path.isfile(emailErrorContent) == False:
            self.log.info(
                "capture.sendCaptureError(): Using to find message file: %(emailErrorContent)s"
            ...
            emailErrorContent = (
                ...
                + "en_US.utf8/"
                + self.dirLocaleMessage
                + "emailErrorContent.txt"
            )
            emailErrorSubject = (
                self.dirLocale
                + "en_US.utf8/"
                + self.dirLocaleMessage
                + "emailErrorSubject.txt"
            )
        self.log.info(
            "capture.sendCaptureError(): Using message subject file: %(emailErrorSubject)s"
            % {"emailErrorSubject": emailErrorSubject}
        )
        self.log.info(
            "capture.sendCaptureError(): Using message content file: %(emailErrorContent)s"
            % {"emailErrorContent": emailErrorContent}
        )
        if os.path.isfile(emailErrorContent) and os.path.isfile(emailErrorSubject):
            ...
            emailSubject = emailSubject.replace(
                "#LASTCAPTUREDATE#",
                self.captureUtils.formatDateLegend(
                    lastSuccessCapture, self.configSource.getConfig("cfgimgdateformat")
                ),
            )
    ...
    def sendCaptureStats(self):
        ...
        self.log.debug("captureEmails.sendCaptureStats(): Start")
        emailSent = 0
        ...
        for liststatsfile in sorted(os.listdir(self.dirStats), reverse=True):
            if (
                liststatsfile[:8] == "capture-"
                and liststatsfile != "capture-" + currentDay + ".txt"
                and emailSent == 0
            ):  # We don't want to take in consideration current day
                if os.path.isfile(
                    self.dirCache
                    + "source"
                    ...
                    self.log.info(
                        "captureEmails.sendCaptureStats(): Email Capture Stats: Email already sent, taking no action"
                    )
                    emailSent = 1
                else:
                    self.log.info(
                        "captureEmails.sendCaptureStats(): Preparation of an email alert about the error"
                    )
                    emailContentFile = (
                        self.dirCurrentLocaleMessages + "emailCaptureStatsContent.txt"
                    )
                    emailSubjectFile = (
                        self.dirCurrentLocaleMessages + "emailCaptureStatsSubject.txt"
                    )
                    if os.path.isfile(emailContentFile) == False:
                        self.log.info(
                            "capture.sendCaptureError(): Using to find message file: %(emailContentFile)s"
                        ...
                        emailContentFile = (
                            ...
                            + "en_US.utf8/"
                            + self.dirLocaleMessage
                            + "emailCaptureStatsContent.txt"
                        )
                        emailSubjectFile = (
                            self.dirLocale
                            + "en_US.utf8/"
                            + self.dirLocaleMessage
                            + "emailCaptureStatsSubject.txt"
                        )
                    self.log.info(
                        "capture.sendCaptureError(): Using message subject file: %(emailSubjectFile)s"
                        % {"emailSubjectFile": emailSubjectFile}
                    )
                    self.log.info(
                        "capture.sendCaptureError(): Using message content file: %(emailContentFile)s"
                        % {"emailContentFile": emailContentFile}
                    )
                    if os.path.isfile(emailContentFile) and os.path.isfile(
                        emailSubjectFile
                    ):
                        ...
                        captureStatsFile = Config(
                            self.log, self.dirStats + liststatsfile
                        )
                        ...
                        emailSubject = emailSubject.replace(
                            "#CAPTURESUCCESS#",
                            str(captureStatsFile.getStat("CaptureSuccess")),
                        )
                        emailSubject = emailSubject.replace(
                            "#CAPTUREREQUEST#",
                            str(captureStatsFile.getStat("CaptureRequest")),
                        )
                        ...
                        emailContent = emailContent.replace(
                            "#MAINFTPSUCCESS#",
                            str(captureStatsFile.getStat("MainFTPSuccess")),
                        )
                        emailContent = emailContent.replace(
                            "#MAINFTPREQUEST#",
                            str(captureStatsFile.getStat("MainFTPRequest")),
                        )
                        emailContent = emailContent.replace(
                            "#SECONDFTPSUCCESS#",
                            str(captureStatsFile.getStat("SecondFTPSuccess")),
                        )
                        emailContent = emailContent.replace(
                            "#SECONDFTPREQUEST#",
                            str(captureStatsFile.getStat("SecondFTPRequest")),
                        )
                        emailContent = emailContent.replace(
                            "#HOTLINKFTPSUCCESS#",
                            str(captureStatsFile.getStat("HotlinkFTPSuccess")),
                        )
                        emailContent = emailContent.replace(
                            "#HOTLINKFTPREQUEST#",
                            str(captureStatsFile.getStat("HotlinkFTPRequest")),
                        )
                        emailContent = emailContent.replace(
                            "#LASTCAPTURE#",
                        ...
                        newEmail = Email(
                            self.log,
                            dir_emails=self.configPaths.getConfig("parameters")[
                                "dir_emails"
                            ],
                            dir_schemas=self.configPaths.getConfig("parameters")[
                                "dir_schemas"
                            ],
                        )
                        newEmail.field_from = {
