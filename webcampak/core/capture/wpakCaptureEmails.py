...
class captureEmails(object):
    ...
    def sendCaptureSuccess(self, captureFilename=None):
        ...
        self.log.info("capture.sendCaptureSuccess(): " + _("Using message subject file: %(emailSuccessSubject)s") % {
            'emailSuccessSubject': emailSuccessSubject})
        self.log.info("capture.sendCaptureSuccess(): " + _("Using message content file: %(emailSuccessContent)s") % {
    ...
    def sendCaptureError(self, sendEmailErrorCount, lastSuccessCapture):
        ...
        if os.path.isfile(emailErrorContent) == False:
            self.log.info("capture.sendCaptureError(): " + _("Using to find message file: %(emailErrorContent)s") % {
        ...
        self.log.info("capture.sendCaptureError(): " + _("Using message subject file: %(emailErrorSubject)s") % {
            'emailErrorSubject': emailErrorSubject})
        self.log.info("capture.sendCaptureError(): " + _("Using message content file: %(emailErrorContent)s") % {
            'emailErrorContent': emailErrorContent})
        if os.path.isfile(emailErrorContent) and os.path.isfile(emailErrorSubject):
            ...
            emailSubject = emailSubject.replace("#CURRENTHOSTNAME#", socket.gethostname())
            emailSubject = emailSubject.replace("#CURRENTSOURCE#", self.currentSourceId)
            emailSubject = emailSubject.replace("#LASTCAPTUREDATE#",
                                                self.captureUtils.formatDateLegend(lastSuccessCapture,
    ...
    def sendCaptureStats(self):
        ...
        for liststatsfile in sorted(os.listdir(self.dirStats), reverse=True):
            if liststatsfile[
               :8] == "capture-" and liststatsfile != "capture-" + currentDay + ".txt" and emailSent == 0:  # We don't want to take in consideration current day
                if os.path.isfile(self.dirCache + "source" + self.currentSourceId + "-" + liststatsfile[
                                                                                          8:16] + "-statsemail"):
                    self.log.info("captureEmails.sendCaptureStats(): " + _(
                        "Email Capture Stats: Email already sent, taking no action"))
                    emailSent = 1
                else:
                    self.log.info(
                        "captureEmails.sendCaptureStats(): " + _("Preparation of an email alert about the error"))
                    ...
                    if os.path.isfile(emailContentFile) == False:
                        self.log.info(
                            "capture.sendCaptureError(): " + _("Using to find message file: %(emailContentFile)s") % {
                    ...
                    self.log.info(
                        "capture.sendCaptureError(): " + _("Using message subject file: %(emailSubjectFile)s") % {
                            'emailSubjectFile': emailSubjectFile})
                    self.log.info(
                        "capture.sendCaptureError(): " + _("Using message content file: %(emailContentFile)s") % {
                            'emailContentFile': emailContentFile})
                    if os.path.isfile(emailContentFile) and os.path.isfile(emailSubjectFile):
                        ...
                        emailSubject = emailSubject.replace("#CURRENTHOSTNAME#", socket.gethostname())
                        emailSubject = emailSubject.replace("#CURRENTSOURCE#", self.currentSourceId)
                        emailSubject = emailSubject.replace("#CAPTURESUCCESS#",
                                                            str(captureStatsFile.getStat('CaptureSuccess')))
                        emailSubject = emailSubject.replace("#CAPTUREREQUEST#",
                                                            str(captureStatsFile.getStat('CaptureRequest')))

                        emailContent = emailContent.replace("#CURRENTSOURCE#", self.currentSourceId)
                        emailContent = emailContent.replace("#CAPTURESUCCESS#",
                                                            str(captureStatsFile.getStat('CaptureSuccess')))
                        emailContent = emailContent.replace("#CAPTUREREQUEST#",
                                                            str(captureStatsFile.getStat('CaptureRequest')))
                        emailContent = emailContent.replace("#MAINFTPSUCCESS#",
                                                            str(captureStatsFile.getStat('MainFTPSuccess')))
                        emailContent = emailContent.replace("#MAINFTPREQUEST#",
                                                            str(captureStatsFile.getStat('MainFTPRequest')))
                        emailContent = emailContent.replace("#SECONDFTPSUCCESS#",
                                                            str(captureStatsFile.getStat('SecondFTPSuccess')))
                        emailContent = emailContent.replace("#SECONDFTPREQUEST#",
                                                            str(captureStatsFile.getStat('SecondFTPRequest')))
                        emailContent = emailContent.replace("#HOTLINKFTPSUCCESS#",
                                                            str(captureStatsFile.getStat('HotlinkFTPSuccess')))
                        emailContent = emailContent.replace("#HOTLINKFTPREQUEST#",
