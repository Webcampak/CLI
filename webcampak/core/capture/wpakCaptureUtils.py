...
class captureUtils(object):
    ...
    def modifyPictures(self, createHotlink):
        ...
        if self.configSource.getConfig('cfgrotateactivate') == "yes":
            self.pictureTransformations.rotate(self.configSource.getConfig('cfgrotateangle'))
        else:
        ...
        if self.configSource.getConfig('cfgpicwatermarkactivate') == "yes":
            watermarkFile = None
            if os.path.isfile(self.dirCurrentSourceWatermarkDir + self.configSource.getConfig("cfgpicwatermarkfile")):
