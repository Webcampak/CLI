...
class captureObj(object):
    ...
    def setCaptureFile(self, captureFile):
        self.log.info(
            "captureObj.setCaptureFile(): Capture file set to: %(captureFile)s"
    ...
    def initCapture(self):
        ...
        self.log.debug("captureObj.initCapture(): Start")
        self.lastCapture = {}
    ...
    def getLastCaptureTime(self):
        ...
        self.log.debug("captureObj.getLastCaptureTime(): Start")
        try:
            lastCaptureTime = dateutil.parser.parse(self.getCaptureValue("captureDate"))
            self.log.info(
                "captureObj.getLastCaptureTime(): Last capture time: %(lastCaptureTime)s"
    ...
    def loadCaptureFile(self):
        ...
        self.log.debug("captureObj.loadCaptureFile(): Start")
    ...
    def writeCaptureFile(self):
        ...
        self.log.debug("captureObj.writeCaptureFile(): Start")
        if self.writeJsonFile(self.captureFile, self.getCapture()) == True:
            self.log.info(
                "captureObj.writeCaptureFile(): Successfully saved last capture file to: %(captureFile)s"
                % {"captureFile": str(self.captureFile)}
            )
            return True
        else:
            self.log.error(
                "captureObj.writeCaptureFile(): Error saving last capture file"
            )
            return False

    def archiveCaptureFile(self):
        """Append the content of the object into a log file containing previous captures"""
        self.log.debug("captureObj.archiveCaptureFile(): Start")
        if self.archiveJsonFile(self.fileCaptureLog, self.getCapture()) == True:
            self.log.info(
                "captureObj.archiveCaptureFile(): Successfully archived capture file to: %(captureFile)s"
                % {"captureFile": str(self.fileCaptureLog)}
            )
            return True
        else:
            self.log.error(
                "captureObj.archiveCaptureFile(): Error saving last capture file"
            )
            return False

    def loadJsonFile(self, jsonFile):
        """Loads the content of a JSON file"""
        self.log.debug("captureObj.loadJsonFile(): Start")
        if os.path.isfile(jsonFile):
            self.log.info(
                "captureObj.loadJsonFile(): Load JSON file into memory: %(jsonFile)s"
                % {"jsonFile": jsonFile}
            )
    ...
    def writeJsonFile(self, jsonFile, jsonContent):
        ...
        self.log.info(
            "captureObj.writeJsonFile(): Writing to: %(jsonFile)s"
            % {"jsonFile": jsonFile}
        )
    ...
    def archiveJsonFile(self, jsonFile, jsonContent):
        ...
        self.log.info(
            "captureObj.archiveJsonFile(): Writing to: %(jsonFile)s"
            % {"jsonFile": jsonFile}
        )
        if fileUtils.CheckFilepath(jsonFile) != "":
