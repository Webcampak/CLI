...
class sensorsObj(object):
    ...
    def initSensors(self):
        ...
        self.log.debug("sensorsObj.initSensors(): Start")
        self.lastSensors = {}
    ...
    def archiveSensorsFile(self):
        ...
        self.log.debug("sensorsObj.archiveSensorsFile(): Start")
        if self.archiveJsonFile(self.fileSensorsLog, self.getSensors()) == True:
            self.log.info(
                "sensorsObj.archiveSensorsFile(): Successfully archived sensor file to: %(captureFile)s"
                % {"captureFile": str(self.fileSensorsLog)}
            )
            return True
        else:
            self.log.error(
                "sensorsObj.archiveSensorsFile(): Error saving last capture file"
            )
            return False

    def loadJsonFile(self, jsonFile):
        """Loads the content of a JSON file"""
        self.log.debug("sensorsObj.loadJsonFile(): Start")
        if os.path.isfile(jsonFile):
            self.log.info(
                "sensorsObj.loadJsonFile(): Load JSON file into memory: %(jsonFile)s"
                % {"jsonFile": jsonFile}
            )
    ...
    def writeJsonFile(self, jsonFile, jsonContent):
        ...
        self.log.info(
            "sensorsObj.writeJsonFile(): Writing to: %(jsonFile)s"
            % {"jsonFile": jsonFile}
        )
    ...
    def archiveJsonFile(self, jsonFile, jsonContent):
        ...
        self.log.info(
            "sensorsObj.archiveJsonFile(): Writing to: %(jsonFile)s"
            % {"jsonFile": jsonFile}
        )
        if fileUtils.CheckFilepath(jsonFile) != "":
