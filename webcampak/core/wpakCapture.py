...
class Capture(object):
    ...
    def __init__(self, log, appConfig, config_dir, sourceId):
        ...
        self.dirCurrentSourceCapture = (
            self.dirSources
            + "source"
            + self.currentSourceId
            + "/"
            + self.dirSourceCapture
        )
        ...
        self.configSource = Config(
            self.log, self.dirEtc + "config-source" + str(self.getSourceId()) + ".cfg"
        )
        self.configSourceFTP = Config(
            self.log,
            self.dirEtc
            + "config-source"
        ...
        self.log.info(
            "capture(): Set Capture Time to script start time (default at script startup)"
        )
        ...
        fileCaptureLog = (
            self.dirCurrentSourceCapture
            + self.getCaptureTime().strftime("%Y%m%d")
            + ".jsonl"
        )

        self.log.info(
            "capture(): Create Capture Status object and set script start date"
        )
        ...
        self.currentCaptureDetails.setCaptureValue(
            "scriptStartDate", self.getScriptStartTime().isoformat()
        )

        self.log.info("capture(): Load previous Capture Status Object (if available)")
        ...
        self.log.info(
            "capture(): Initializing the following capture driver: %(captureDriver)s"
        ...
        if self.configSource.getConfig("cfgsourcetype") == "gphoto":
            ...
            self.configSource.getConfig("cfgsourcetype") == "wpak"
            and self.configSource.getConfig("cfgsourcewpaktype") == "rec"
        ):
            ...
            self.configSource.getConfig("cfgsourcetype") == "wpak"
            and self.configSource.getConfig("cfgsourcewpaktype") == "get"
        ):
    ...
    def setScriptStartTime(self, scriptStartTime):
        self.log.info(
            "capture.setScriptStartTime(): Script Start Time set to: %(scriptStartTime)s"
    ...
    def setCaptureTime(self, captureTime=None):
        ...
        self.log.info(
            "capture.setCaptureTime(): Capture Time set to: %(captureTime)s"
    ...
    def run(self):
        ...
        self.log.info(
            "capture.run(): Initiate capture process for source: %(currentSourceId)s"
        ...
        if self.configSource.getConfig("cfgcapturedelay") != "0":
            self.log.info(
                "capture.run(): Delaying capture by %(CaptureDelay)s seconds."
        ...
        if self.configSource.getConfig("cfgnocapture") == "yes":
            self.log.info(
                "capture.run(): Capture manually disabled via administration panel"
            )
        elif self.configSource.getConfig("cfgsourceactive") != "yes":
            self.log.info(
                "capture.run(): Source is not active, not proceeding with capture"
            )
        elif self.captureUtils.isWithinTimeframe() == False:
            self.log.info(
                "capture.run(): Capture calendar is active but capture not in the correct timeframe"
            )
        elif self.captureUtils.checkInterval() == False:
            self.log.info(
                "capture.run(): Not enough time since last picture was captured, not proceeding"
            )
        else:
            ...
            if capturedPictures != False:
                for currentPicture in capturedPictures:
                    self.log.info(
                        "capture.run(): Begin processing of picture: %(currentPicture)s"
                        % {"currentPicture": currentPicture}
                    )

                    # Set picture filename
                    self.setCaptureFilename(
                    ...
                    if (
                        processedPicturesCount == 0
                        or self.configSource.getConfig("cfgsourcecamiplimiterotation")
                        != "yes"
                    ):
                    ...
                    self.captureUtils.sendPicture(
                        self.configSource.getConfig("cfgftpmainserverid"),
                    ...
                    self.captureUtils.sendPicture(
                        self.configSource.getConfig("cfgftpsecondserverid"),
                    ...
                    if self.configSource.getConfig("cfgcopymainenable") == "yes":
                        self.captureUtils.copyPicture(
                            self.configSource.getConfig("cfgcopymainsourceid"),
                    ...
                    if self.configSource.getConfig("cfgcopysecondenable") == "yes":
                        self.captureUtils.copyPicture(
                            self.configSource.getConfig("cfgcopysecondsourceid"),
                    ...
                    storedJpgSize = self.captureUtils.getArchivedSize(
                        self.getCaptureFilename(), "jpg"
                    )
                    storedRawSize = self.captureUtils.getArchivedSize(
                        self.getCaptureFilename(), "raw"
                    )
                    self.currentCaptureDetails.setCaptureValue(
                        "storedJpgSize",
                        self.currentCaptureDetails.getCaptureValue("storedJpgSize")
                        + storedJpgSize,
                    )
                    self.currentCaptureDetails.setCaptureValue(
                        "storedRawSize",
                        self.currentCaptureDetails.getCaptureValue("storedRawSize")
                        + storedRawSize,
                    )
                    self.currentCaptureDetails.setCaptureValue(
                        "totalCaptureSize",
                        self.currentCaptureDetails.getCaptureValue("totalCaptureSize")
                        + int(storedJpgSize + storedRawSize),
                    )
                    processedPicturesCount = processedPicturesCount + 1

                self.log.info("capture.run(): Capture process completed")
                self.currentCaptureDetails.setCaptureValue("captureSuccess", True)
                if os.path.isfile(
                    self.dirCache + "source" + self.currentSourceId + "-errorcount"
                ):
                    os.remove(
                        self.dirCache + "source" + self.currentSourceId + "-errorcount"
                    )
            else:
                self.log.info("capture.run(): Unable to capture picture")
                self.captureUtils.generateFailedCaptureHotlink()
                self.currentCaptureDetails.setCaptureValue("captureSuccess", False)
                self.captureUtils.setCustomCounter(
                    "errorcount",
                    int(self.captureUtils.getCustomCounter("errorcount")) + 1,
                )
            ...
            if (
                self.configGeneral.getConfig("cfgphidgetactivate") == "yes"
                and self.configSource.getConfig("cfgphidgetactivate") == "yes"
            ):
                capturedSensors = capturePhidget(self).capture()
                currentSensorsDetails = sensorsObj(self.log, fileCaptureLog)
                currentSensorsDetails.setSensorsValue(
                    "date", self.getCaptureTime().isoformat()
                )
                currentSensorsDetails.setSensorsValue("sensors", capturedSensors)
                # Record capture interval
                sourceCaptureInterval = int(
                    self.configSource.getConfig("cfgcroncapturevalue")
                )
                if self.configSource.getConfig("cfgcroncaptureinterval") == "minutes":
                    sourceCaptureInterval = (
                        int(self.configSource.getConfig("cfgcroncapturevalue")) * 60
                    )
            ...
            totalCaptureTime = int(
                (scriptEndDate - self.getScriptStartTime()).total_seconds() * 1000
            )
            self.log.info(
                "capture.run(): Capture: Overall capture time: %(TotalCaptureTime)s ms"
                % {"TotalCaptureTime": str(totalCaptureTime)}
            )
            self.currentCaptureDetails.setCaptureValue(
                "scriptEndDate", scriptEndDate.isoformat()
            )
            self.currentCaptureDetails.setCaptureValue(
                "scriptRuntime", totalCaptureTime
            )
            self.currentCaptureDetails.setCaptureValue(
                "processedPicturesCount", processedPicturesCount
            )
