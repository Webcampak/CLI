...
class Capture(object):
    ...
    def __init__(self, log, appConfig, config_dir, sourceId):    
        ...
        self.log.info("capture(): " + _("Initializing the following capture driver: %(captureDriver)s") % {'captureDriver': self.configSource.getConfig('cfgsourcetype')} )                        
        ...
        elif self.configSource.getConfig('cfgsourcetype') == "ipcam" or (self.configSource.getConfig('cfgsourcetype') == "wpak" and self.configSource.getConfig('cfgsourcewpaktype') == "rec"):   
        ...
        elif self.configSource.getConfig('cfgsourcetype') == "wpak" and self.configSource.getConfig('cfgsourcewpaktype') == "get":  
    ...
    def setScriptStartTime(self, scriptStartTime):
        self.log.info("capture.setScriptStartTime(): " + _("Script Start Time set to: %(scriptStartTime)s") % {'scriptStartTime': scriptStartTime.isoformat()} )        
    ...
    def setCaptureTime(self, captureTime = None):
        ...
        self.log.info("capture.setCaptureTime(): " + _("Capture Time set to: %(captureTime)s") % {'captureTime': str(self.captureTime)} )                    
        ...
        if self.configSource.getConfig('cfgnocapture') == "yes":
            ...
            if capturedPictures != False:
                for currentPicture in capturedPictures:
                    self.log.info("capture.run(): " + _("Begin processing of picture: %(currentPicture)s") % {'currentPicture': currentPicture}) 
                    ...
                    if processedPicturesCount == 0 or self.configSource.getConfig('cfgsourcecamiplimiterotation') != "yes":
                    ...
                    self.captureUtils.sendPicture(self.configSource.getConfig('cfgftpmainserverid'), self.configSource.getConfig('cfgftpmainserverretry'), self.configSource.getConfig('cfgftpmainserverraw'), self.captureFilename)
                    
                    # Send file to second remote FTP Server
                    self.captureUtils.sendPicture(self.configSource.getConfig('cfgftpsecondserverid'), self.configSource.getConfig('cfgftpsecondserverretry'), self.configSource.getConfig('cfgftpsecondserverraw'), self.captureFilename)
                    
                    # Copy file to first internal source
                    if self.configSource.getConfig('cfgcopymainenable') == "yes":
                        self.captureUtils.copyPicture(self.configSource.getConfig('cfgcopymainsourceid'), self.configSource.getConfig('cfgcopymainsourceraw'), self.captureFilename)
                    
                    # Copy file to second internal source
                    if self.configSource.getConfig('cfgcopysecondenable') == "yes":                    
                        self.captureUtils.copyPicture(self.configSource.getConfig('cfgcopysecondsourceid'), self.configSource.getConfig('cfgcopysecondsourceraw'), self.captureFilename)
                    ...
                    self.currentCaptureDetails.setCaptureValue('storedJpgSize', self.currentCaptureDetails.getCaptureValue('storedJpgSize') + storedJpgSize)  
                    self.currentCaptureDetails.setCaptureValue('storedRawSize', self.currentCaptureDetails.getCaptureValue('storedRawSize') + storedRawSize)  
                    self.currentCaptureDetails.setCaptureValue('totalCaptureSize', self.currentCaptureDetails.getCaptureValue('totalCaptureSize') + int(storedJpgSize + storedRawSize))  
                    ...
                    if os.path.isfile(self.dirCache + "source" + self.currentSourceId + "-errorcount"): 
                        ...
                        self.log.info("capture.run(): " + _("Process found that previous capture(s) failed, error count: %(currentErrorCount)s") % {'currentErrorCount': str(currentErrorCount)})  
                        if (currentErrorCount >= int(self.configSource.getConfig('cfgemailalertfailure')) or self.configSource.getConfig('cfgemailalwaysnotify') == "yes") and self.configSource.getConfig('cfgemailerroractivate') == "yes":				
                            self.log.info("capture.run(): " + _("Preparation of an email to inform that the system is back"))	
                            self.captureEmails.sendCaptureSuccess(self.captureFilename)
                        else:
                            self.log.info("capture.run(): " + _("Not enough capture errors to trigger an action. Threshold: %(currentErrorThreshold)s errors") % {'currentErrorThreshold': str(self.configSource.getConfig('cfgemailalertfailure'))})                                                      
                        self.log.info("capture.run(): " + _("Deleting 'errorcount' file for source (to reset error counter)"))														
                ...
                self.log.info("capture.run(): " + _("Previous Error Count was: %(previousErrorCount)s") % {'previousErrorCount': previousErrorCount}) 
            ...
            self.log.info("capture.run(): " + _("Capture: Overall capture time: %(TotalCaptureTime)s ms") % {'TotalCaptureTime': str(totalCaptureTime)} )
