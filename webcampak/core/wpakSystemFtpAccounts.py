...
class systemFtpAccounts:
    ...
    def initGetText(self, dirLocale, cfgsystemlang, cfggettextdomain):
        ...
        try:
            ...
            _ = t.ugettext
            t.install()
            self.log.info("systemFtpAccounts.initGetText(): " + _("Initialized gettext with Domain: %(cfggettextdomain)s - Language: %(cfgsystemlang)s - Path: %(dirLocale)s")
	...
	for scanfile in sorted(os.listdir(self.dirEtc), reverse=True):
            if re.findall('config-source[0-9]+.cfg', scanfile) and scanfile[-1] != "~":
                ...
                self.log.info("systemFtpAccounts.create(): " + _("Identified configuration file: %(scanfile)s Source ID: %(sourceid)s")% {'scanfile': scanfile, 'sourceid': sourceid})                 
                ...
                self.log.info("systemFtpAccounts.create(): " + _("Opening configuration file %(scanfile)s for source: %(sourceid)s, creating ..")% {'scanfile': scanfile, 'sourceid': sourceid})                                     
                ...
                if os.path.exists(self.dirSources + "source" + sourceid + "/") == True:
                    self.log.info("systemFtpAccounts.create(): " + _("Making directory not writable %(directory)s")% {'directory': self.dirSources + "source" + sourceid + "/"})                                                         
                    Command = "chmod -w " + self.dirSources + "source" + sourceid + "/"
                    self.log.info("systemFtpAccounts.create(): " + _("Modifying file permission: %(Command)s") % {'Command': Command})                    
...
# cat /home/francois/webcampak/init/config/vsftpd-source >> /etc/vsftpd/vsftpd_user_conf/source3
	...
	Command = "db5.3_load -T -t hash -f /etc/vsftpd/ftpusers /etc/vsftpd/login.db"
        self.log.info("systemFtpAccounts.create(): " + _("Creation of the database: %(Command)s") % {'Command': Command})                    
