...
class Video(object):
    ...
    def __init__(self, log, appConfig, config_dir, sourceId, videoType):
        ...
        self.dirCurrentSourceVideos = (
            ...
            + "/"
            + self.configPaths.getConfig("parameters")["dir_source_videos"]
        )
        ...
        self.dirCurrentSourceResourcesVideos = (
            self.dirSources
            + "source"
            + self.currentSourceId
            + "/"
            + self.configPaths.getConfig("parameters")["dir_source_resources_videos"]
        )
        ...
        self.configSource = Config(
            self.log, self.dirEtc + "config-source" + str(self.currentSourceId) + ".cfg"
        )
        ...
        fileVideoLog = (
            self.dirCurrentSourceResourcesVideos
            + self.getScriptStartTime().strftime("%Y%m")
            + ".jsonl"
        )
        self.currentVideoDetails = videoObj(self.log, fileVideoLog)
        self.currentVideoDetails.setVideoValue(
    ...
    def setScriptStartTime(self, scriptStartTime):
        self.log.info(
            "video.setScriptStartTime(): Script Start Time set to: %(scriptStartTime)s"
    ...
    def run(self):
        ...
        if self.videoUtils.isCreationAllowed() == False:
            self.log.info(
                "video.run(): Video creation is currently not allowed for the source"
            )
        else:
            self.log.info("video.run(): Video creation allowed, starting the process")
            if self.videoType == "videocustom" or self.videoType == "videopost":
                ...
                self.setVideoFilename(
                    self.getScriptStartTime().strftime("%Y%m%d")
                    + "_"
                    + self.configSourceVideo.getConfig("cfgcustomvidname")
                )

            self.setProcessVideoDir(
                self.dirCurrentSourcePictures + "process-" + self.videoType + "/"
            )
            self.setProcessVideoFiles(
                self.dirCurrentSourcePictures + "process-" + self.videoType + "/20*.jpg"
            )
            ...
            if filesCopied > 0:
                for scanPictureFile in sorted(
                    os.listdir(self.getProcessVideoDir()), reverse=True
                ):
                    if (
                        self.fileUtils.CheckJpegFile(
                            self.getProcessVideoDir() + scanPictureFile
                        )
                        == True
                    ):
                        self.log.info(
                        ...
                        if (
                            self.videoType == "videopost"
                            and self.configSourceVideo.getConfig(
                                "cfgtransitionactivate"
                            )
                            != "yes"
                        ):
                            if (
                                self.configSourceVideo.getConfig("cfgrotateactivate")
                                == "yes"
                                or self.configSourceVideo.getConfig("cfgcropactivate")
                                == "yes"
                                or self.configSourceVideo.getConfig(
                                    "cfgvideosizeactivate"
                                )
                                == "yes"
                                or self.configSourceVideo.getConfig(
                                    "cfgwatermarkactivate"
                                )
                                == "yes"
                                or self.configSourceVideo.getConfig(
                                    "cfgvideopreimagemagicktxt"
                                )
                                == "yes"
                                or self.configSourceVideo.getConfig("cfgvideoeffect")
                                != "no"
                                or self.configSourceVideo.getConfig(
                                    "cfgthumbnailactivate"
                                )
                                != "no"
                            ):
                                self.videoUtils.modifyPicturesPost(
                                    self.getProcessVideoDir() + scanPictureFile
                                )
                            if (
                                ...
                                if os.path.exists(
                                    ...
                                    self.log.info(
                                        "video.run(): Error: File already exists %(DestinationFile)s"
                                        % {
                                    ...
                                    self.log.info(
                                        "video.run(): Copy file to: %(DestinationFile)s"
                                        % {
                                            "DestinationFile": self.sourceDestinationDirectory
                                            + scanPictureFile
                                        }
                            ...
                            self.videoType == "videocustom" or self.videoType == "video"
                        ):
                            self.videoUtils.modifyPictures(
                                self.getProcessVideoDir() + scanPictureFile
                            )
                            CreationError = False
                    else:
                        self.log.info(
                            "video.run(): Video: %(VideoType)s: Picture format incorrect, deleting ...."
                ...
                if (
                    self.videoType == "videopost"
                    and self.configSourceVideo.getConfig("cfgtransitionactivate")
                    == "yes"
                ):
                    self.videoUtils.transitionPictures()

                self.log.info(
                    "video.run(): All pictures have been copied to target directory"
                    % {"VideoType": self.videoType}
                )

                for videoformats in self.configGeneral.getConfig("cfgvideoformats"):
                    self.log.info(
                        "video.run(): Starting to process video format: %(videoformats)s"
                        % {"videoformats": videoformats}
                    )
                    formatDictionary = {}
                    formatDictionary["name"] = videoformats
                    formatStartTime = self.timeUtils.getCurrentSourceTime(
                        self.configSource
                    )
                    if (
                        self.configSourceVideo.getConfig(
                            "cfgvideocodecH264" + videoformats + "create"
                        )
                        == "yes"
                        ...
                        if TargetVideoFilename != False:
                            self.log.info(
                                "video.run(): Processing completed for: %(TargetVideoFilename)s"
                                % {"TargetVideoFilename": TargetVideoFilename}
                            )
                            if os.path.isfile(
                                self.dirCurrentSourceVideos + TargetVideoFilename
                            ):
                                formatDictionary["avi"] = os.path.getsize(
                                    self.dirCurrentSourceVideos + TargetVideoFilename
                                )
                            if os.path.isfile(
                                self.dirCurrentSourceVideos
                                ...
                                formatDictionary["mp4"] = os.path.getsize(
                                    self.dirCurrentSourceVideos
                                    + TargetVideoFilename
                                    + ".mp4"
                                )
                            if self.videoType == "video":
                                self.videoUtils.sendVideos(
                                    TargetVideoFilename, videoformats
                                )
                            if (
                                self.videoType == "videocustom"
                                and self.configSourceVideo.getConfig(
                                    "cfgemailmovieactivate"
                                )
                                == "yes"
                            ):
                            ...
                            self.log.error(
                                "video.run(): Error for creating video for format: %(videoformats)s"
                                % {"videoformats": videoformats}
                            )
                    formatEndTime = self.timeUtils.getCurrentSourceTime(
                        self.configSource
                    )
                    totalProcessingTime = int(
                        (formatEndTime - formatStartTime).total_seconds() * 1000
                    )
                    formatDictionary["runtime"] = totalProcessingTime
                    self.currentVideoDetails.addFormat(formatDictionary)

                self.log.info("video.run(): Video processing completed")
            else:
                self.log.info("video.run(): No files available for processing, exiting")
            ...
            self.log.info(
                "video.run(): Video: Overall processing time: %(TotalCaptureTime)s ms"
