...
class Video:
    ...
    def __init__(self, log, appConfig, config_dir, sourceId, videoType):
        ...
        self.dirCurrentSourceVideos = self.dirSources + 'source' + self.currentSourceId + '/' + \
        ...
        self.dirCurrentSourceResourcesVideos = self.dirSources + 'source' + self.currentSourceId + '/' + \
        ...
        self.configSourceVideo = Config(self.log, self.dirEtc + 'config-source' + str(
            self.currentSourceId) + '-' + self.videoType + '.cfg')
    ...
    def initGetText(self, dirLocale, cfgsystemlang, cfggettextdomain):
        ...
        try:
            ...
            _ = t.ugettext
            t.install()
            self.log.info("video.initGetText(): " + _(
    ...
    def setScriptStartTime(self, scriptStartTime):
        self.log.info("video.setScriptStartTime(): " + _("Script Start Time set to: %(scriptStartTime)s") % {
        ...
        if self.videoUtils.isCreationAllowed() == False:
            ...
            if self.videoType == "videocustom" or self.videoType == "videopost":
                ...
                self.setVideoFilename(
                    self.getScriptStartTime().strftime("%Y%m%d") + "_" + self.configSourceVideo.getConfig(
                        'cfgcustomvidname'))
            ...
            if filesCopied > 0:
                for scanPictureFile in sorted(os.listdir(self.getProcessVideoDir()), reverse=True):
                    if self.fileUtils.CheckJpegFile(self.getProcessVideoDir() + scanPictureFile) == True:
                        self.log.info("video.run(): " + _(
                            "---------------------------------------------------------------------------------"))
                        if self.videoType == "videopost" and self.configSourceVideo.getConfig(
                                'cfgtransitionactivate') != "yes":
                            if self.configSourceVideo.getConfig(
                                    'cfgrotateactivate') == "yes" or self.configSourceVideo.getConfig(
                                    'cfgcropactivate') == "yes" or self.configSourceVideo.getConfig(
                                    'cfgvideosizeactivate') == "yes" or self.configSourceVideo.getConfig(
                                    'cfgwatermarkactivate') == "yes" or self.configSourceVideo.getConfig(
                                    'cfgvideopreimagemagicktxt') == "yes" or self.configSourceVideo.getConfig(
                                    'cfgvideoeffect') != "no" or self.configSourceVideo.getConfig(
                            ...
                            if self.configSourceVideo.getConfig("cfgmovefilestosource") != "no":
                                ...
                                self.sourceDestinationDirectory = self.dirSources + 'source' + self.configSourceVideo.getConfig(
                                    "cfgmovefilestosource") + '/' + self.configPaths.getConfig('parameters')[
                                                                      'dir_source_pictures'] + scanPictureFile[
                                                                                               0:8] + "/"
                                self.fileUtils.CheckDir(self.sourceDestinationDirectory)
                                if os.path.exists(self.sourceDestinationDirectory + scanPictureFile):
                                    self.log.info(
                                        "video.run(): " + _("Error: File already exists %(DestinationFile)s") % {
                                            'DestinationFile': self.sourceDestinationDirectory + scanPictureFile})
                                else:
                                    self.log.info("video.run(): " + _("Copy file to: %(DestinationFile)s") % {
                                        'DestinationFile': self.sourceDestinationDirectory + scanPictureFile})
                                    shutil.copy(self.getProcessVideoDir() + scanPictureFile,
                        ...
                        self.log.info(
                            "video.run(): " + _("Video: %(VideoType)s: Picture format incorrect, deleting ....") % {
                ...
                self.log.info("video.run(): " + _("All pictures have been copied to target directory") % {
                    'VideoType': self.videoType})

                for videoformats in self.configGeneral.getConfig('cfgvideoformats'):
                    self.log.info("video.run(): " + _("Starting to process video format: %(videoformats)s") % {
                        'videoformats': videoformats})
                    formatDictionary = {}
                    formatDictionary['name'] = videoformats
                    formatStartTime = self.timeUtils.getCurrentSourceTime(self.configSource)
                    if self.configSourceVideo.getConfig(
                                            "cfgvideocodecH264" + videoformats + "create") == "yes" and self.videoType != "videopost":
                        TargetVideoFilename = self.videoUtils.createVideos(videoformats)
                        if TargetVideoFilename != False:
                            self.log.info("video.run(): " + _("Processing completed for: %(TargetVideoFilename)s") % {
                            ...
                            if self.videoType == "videocustom" and self.configSourceVideo.getConfig(
                                    "cfgemailmovieactivate") == "yes":
                                self.videoEmails.sendVideoSuccess(TargetVideoFilename)
                        else:
                            self.log.error(
                                "video.run(): " + _("Error for creating video for format: %(videoformats)s") % {
                                    'videoformats': videoformats})
            ...
            self.log.info("video.run(): " + _("Video: Overall processing time: %(TotalCaptureTime)s ms") % {
