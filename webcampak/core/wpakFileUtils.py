        ...
        for listpictdir in sorted(os.listdir(sourcePicturesDirectory), reverse=True):
            if listpictdir[:2] == "20" and os.path.isdir(sourcePicturesDirectory + listpictdir):
                for listpictfiles in sorted(os.listdir(sourcePicturesDirectory + listpictdir), reverse=True):
                    if listpictfiles[:2] == "20" and self.CheckJpegFile(sourcePicturesDirectory + listpictdir + "/" + listpictfiles) == True:
                        self.log.info("fileUtils.SecondsSinceLastCapture(): " + _("Last Picture: %(lastScannedPicture)s") % {'lastScannedPicture': str(sourcePicturesDirectory + listpictdir + "/" + listpictfiles)} )		
                        ...
                        if Timezone != "": # Update the timezone from UTC to the source's timezone
                            ...
                            self.log.info("fileUtils.SecondsSinceLastCapture(): " + _("File Date: %(fileDate)s") % {'fileDate': str(fileDateTime) } )		                            
    ...
    def SecondsBetweenPictures(Directory, CurrentFile):
        ...
        for listpictfiles in sorted(os.listdir(Directory), reverse=False):
            if listpictfiles[:2] == "20" and self.CheckJpegFile(Directory + "/" + listpictfiles) == True:
                ...
                self.log.info("fileUtils.SecondsSinceLastCapture(): " + _("Time difference in seconds %(timedifference)s") % {'timedifference': str(timedifference.seconds) } )		                            
