...
class reportsDaily(object):
    ...
    def run(self):
        ...
        self.log.info("reportsDaily.run(): Initiate reports creation")
        emailReports = {}
        for currentSource in self.sourcesUtils.getActiveSourcesIds():
            self.log.info(
                "reportsDaily.run(): Processing source %(currentSource)s"
                % {"currentSource": str(currentSource)}
            )

            sourceQuota = self.dbUtils.getSourceQuota(currentSource)
            sourceDiskUsage = fileUtils.CheckDirDu(
            ...
            self.log.info(
                "reportsDaily.run(): Source disk usage is: %(sourceDiskUsage)s"
                % {"sourceDiskUsage": str(sourceDiskUsage)}
            )
            if sourceQuota == None or int(sourceQuota) == 0:
                sourceDiskPercentUsed = "n/a"
            else:
                sourceDiskPercentUsed = int(
                    int(sourceDiskUsage) / int(sourceQuota) * 100
                )
            ...
            for currentReportDay in missingReports:
                if (
                    int(self.timeUtils.getCurrentDate().strftime("%Y%m%d"))
                    == currentReportDay
                ):
                    self.log.info(
                        "reportsDaily.run(): Skipping report for current day: %(currentReportDay)s"
                        % {"currentReportDay": str(currentReportDay)}
                    )
                else:
                    currentSourceReport = self.generateReport(
                        currentSource, currentReportDay, sourceSchedule
                    )
                    currentSourceReport["source"] = {
                        "quota": sourceQuota,
                        "usage": sourceDiskUsage,
                        "percentUsed": sourceDiskPercentUsed,
                    }
                    currentSourceReport["disk"] = self.getFileSystemUsage(
                        self.dirSources
                    )
                    dirCurrentSourceReports = (
                        self.dirSources
                        + "source"
                        + str(currentSource)
                        + "/"
                        + self.configPaths.getConfig("parameters")[
                            "dir_source_resources_reports"
                        ]
                    )
                    jsonReportFile = (
                        dirCurrentSourceReports + str(currentReportDay) + ".json"
                    )
                    self.log.info(
                        "reportsDaily.run(): Preparing to save report in %(jsonReportFile)s"
                        % {"jsonReportFile": str(jsonReportFile)}
                    )
                    if self.writeJsonFile(jsonReportFile, currentSourceReport):
                        reportEmail = self.prepareEmailReportContent(
                            currentSource, currentReportDay, currentSourceReport
                        )
                        emailReports[currentSource][currentReportDay] = {
                            "sourceid": currentSource,
                            "reportDay": currentReportDay,
                            "report": currentSourceReport,
                            "reportEmail": reportEmail,
                        }
                    else:
                        self.log.error(
                            "reportsDaily.run(): Error saving report file to disk %(jsonReportFile)s"
                            % {"jsonReportFile": str(jsonReportFile)}
                        )

        self.log.info("reportsDaily.run(): Getting ready to send reports")
    ...
    def sendReportEmail(self, currentUser, emailReports):
        self.log.debug("reportsDaily.sendReportEmail(): Start")
        self.log.info(
            "reportsDaily.sendReportEmail(): Preparing to send email to user %(name)s - email: %(email)s"
        ...
        for currentSource in currentUser["sources"]:
            self.log.info(
                "reportsDaily.sendReportEmail(): Looking for report for source %(sourceid)s"
                % {"sourceid": currentSource["sourceid"]}
            )
            if currentSource["sourceid"] in emailReports:
                for currentDailyReport in sorted(
                    emailReports[currentSource["sourceid"]]
                ):
                    currentSourceId = currentSource["sourceid"]
                    emailReportBody = (
                        emailReportBody
                        + emailReports[currentSource["sourceid"]][currentDailyReport][
                            "reportEmail"
                        ]
                        + "\n -----------------------\n"
                    )
                ...
                self.log.info(
                    "reportsDaily.sendReportEmail(): No report found for source %(sourceid)s"
                    % {"sourceid": currentSource["sourceid"]}
                )
        ...
        if currentSourceId != None:
            configSource = Config(
                self.log, self.dirEtc + "config-source" + str(currentSourceId) + ".cfg"
            )
            dirCurrentLocaleEmails = (
                self.dirLocale
                + configSource.getConfig("cfgsourcelanguage")
                + "/"
                + self.dirLocaleEmails
            )
            ...
            if os.path.isfile(emailReportContentFile) == False:
                emailReportSubjectFile = (
                    self.dirLocale
                    + "en_US.utf8/"
                    + self.dirLocaleEmails
                    + "dailyReportSubject.txt"
                )
                emailReportContentFile = (
                    self.dirLocale
                    + "en_US.utf8/"
                    + self.dirLocaleEmails
                    + "dailyReportContent.txt"
                )
            self.log.info(
                "captureportsDailyre.sendReportEmail(): Using message subject file: %(emailReportSubjectFile)s"
                % {"emailReportSubjectFile": emailReportSubjectFile}
            )
            self.log.info(
                "reportsDaily.sendReportEmail(): Using message content file: %(emailReportContentFile)s"
                % {"emailReportContentFile": emailReportContentFile}
            )
            ...
            emailReportSubject = emailReportSubject.replace(
                "#CURRENTHOSTNAME#", socket.gethostname()
            )
            emailReportSubject = emailReportSubject.replace(
                "#REPORTDATE#", self.timeUtils.getCurrentDate().strftime("%B %d, %Y")
            )
            self.log.info(
                "reportsDaily.sendReportEmail(): Email subject: %(emailReportSubject)s"
                % {"emailReportSubject": emailReportSubject}
            )
            ...
            emailReportContent = emailReportContent.replace(
                "#REPORTBODY#", emailReportBody
            )
            emailReportContent = emailReportContent.replace(
            ...
            self.log.info(
                "reportsDaily.sendReportEmail(): Email Report content: %(emailReportContent)s"
                % {"emailReportContent": "\n" + emailReportContent}
            )
            ...
            newEmail.field_from = {
                "email": self.configGeneral.getConfig("cfgemailsendfrom")
            }
            newEmail.field_to = [
                {"name": currentUser["name"], "email": currentUser["email"]}
            ]
    ...
    def prepareEmailReportContent(
        self, currentSource, currentReportDay, currentSourceReport
    ):
        self.log.debug("reportsDaily.prepareEmailReportContent(): Start")
        self.log.info(
            "reportsDaily.prepareEmailReportContent(): Preparing report content for Source: %(currentSource)s and Day: %(currentReportDay)s"
            % {
                "currentSource": str(currentSource),
                "currentReportDay": str(currentReportDay),
            }
        )
        reportDatetime = datetime.strptime(
            str(currentReportDay) + "120000", "%Y%m%d%H%M%S"
        )
        table = [
            [
                "Source Name (ID)",
                self.dbUtils.getSourceName(currentSource)
                + " ("
                + str(currentSource)
                + ")",
            ],
            ...
            [
                "Captured JPG | RAW",
                str(currentSourceReport["capture"]["JPG"]["count"])
                + " | "
                + str(currentSourceReport["capture"]["RAW"]["count"]),
            ],
        ]
        ...
        if currentSourceReport["source"]["quota"] == None:
            ...
            currentSourceAvailable = int(currentSourceReport["source"]["quota"]) - int(
                int(str(currentSourceReport["source"]["usage"].deocde("UTF-8")))
            )
        ...
        table = [
            [
                "Source",
                fileUtils.sizeof_fmt(currentSourceReport["source"]["usage"]),
                fileUtils.sizeof_fmt(currentSourceReport["source"]["quota"]),
                fileUtils.sizeof_fmt(currentSourceAvailable),
                currentSourceReport["source"]["percentUsed"],
            ],
            [
                "Total Disk",
                fileUtils.sizeof_fmt(currentSourceReport["disk"]["used"]),
    ...
    def generateReport(self, currentSource, currentReportDay, sourceSchedule):
        self.log.debug("reportsDaily.generateReport(): Start")
        self.log.info(
            "reportsDaily.generateReport(): Generating report for Source: %(currentSource)s and Day: %(currentReportDay)s"
            % {
                "currentSource": str(currentSource),
                "currentReportDay": str(currentReportDay),
            }
        ...
        currentJPGs = self.getPicturesStats(
            dirCurrentSourcePictures + str(currentReportDay) + "/"
        )
        currentRAWs = self.getPicturesStats(
            dirCurrentSourcePictures + "raw/" + str(currentReportDay) + "/"
        )
        reportDatetime = datetime.strptime(
            str(currentReportDay) + "120000", "%Y%m%d%H%M%S"
        )
        if sourceSchedule != None:
            scheduleReport = self.compareScheduleWithDisk(
                sourceSchedule, currentJPGs, currentRAWs, reportDatetime
            )
        else:
            scheduleReport = None
    ...
    def compareScheduleWithDisk(
        self, sourceSchedule, currentJPGs, currentRAWs, reportDatetime
    ):
        self.log.debug("reportsDaily.compareScheduleWithDisk(): Start")
        self.log.info(
            "reportsDaily.compareScheduleWithDisk(): Comparing Scedhule with Disk"
        )
        ...
        for hour in range(24):
            for minute in range(59):
                ...
                self.log.debug(
                    "reportsDaily.compareScheduleWithDisk(): Testing timeslot: %(fullHour)s:%(fullMinute)s"
                    % {"fullHour": str(fullHour), "fullMinute": str(fullMinute)}
                )
                if reportDayOfWeek in sourceSchedule:
                    ...
                    if testIdx in currentJPGs["list"]:
                        if (
                            fullHour in sourceSchedule[reportDayOfWeek]
                            and fullMinute in sourceSchedule[reportDayOfWeek][fullHour]
                            and sourceSchedule[reportDayOfWeek][fullHour][fullMinute]
                            == "Y"
                        ):
        ...
        return {
            "onschedule": {
                "JPG": {"list": intersectJPG, "count": len(intersectJPG)},
                "RAW": {"list": intersectRAW, "count": len(intersectRAW)},
            },
            "missing": {
                "JPG": {"list": missingJPG, "count": len(missingJPG)},
                "RAW": {"list": missingRAW, "count": len(missingRAW)},
            },
            "extra": {
                "JPG": {"list": extraJPG, "count": len(extraJPG)},
                "RAW": {"list": extraRAW, "count": len(extraRAW)},
            },
        }

    def getPicturesStats(self, picturesDirectory):
        self.log.debug("reportsDaily.getPicturesStats(): Start")
        self.log.info(
            "reportsDaily.getPicturesStats(): Scanning directory: %(picturesDirectory)s"
        ...
        if os.path.exists(picturesDirectory):
            for currentPicture in os.listdir(picturesDirectory):
                if len(
                    os.path.splitext(os.path.basename(currentPicture))[0]
                ) == 14 and (
                    os.path.splitext(os.path.basename(currentPicture))[1] == ".jpg"
                    or os.path.splitext(os.path.basename(currentPicture))[1] == ".raw"
                ):
                    ...
                    listPictures[pictureIdx] = {
                        "filename": currentPicture,
                        "filesize": pictureSize,
                    }
    ...
    def getMissingReports(self, sourceId):
        ...
        self.log.info("reportsDaily.getMissingReports(): Start")
        dirCurrentSourcePictures = (
            self.dirSources
            + "source"
            + str(sourceId)
        ...
        dirCurrentSourceReports = (
            ...
            + str(sourceId)
            + "/"
            + self.configPaths.getConfig("parameters")["dir_source_resources_reports"]
        )
        missingDays = []
        for picturesDay in os.listdir(dirCurrentSourcePictures):
            if picturesDay[0:2] == "20" and len(picturesDay) == 8:
                if os.path.isfile(dirCurrentSourceReports + picturesDay + ".json"):
                    self.log.info(
                        "reportsDaily.getMissingReports(): Report exists for day: %(picturesDay)s"
                        % {"picturesDay": str(picturesDay)}
                    )
                else:
                    self.log.info(
                        "reportsDaily.getMissingReports(): Report does not exist for day: %(picturesDay)s"
                        % {"picturesDay": str(picturesDay)}
                    )
    ...
    def getSourceSchedule(self, currentSource):
        self.log.debug("reportsDaily.getSourceSchedule(): Start")
        scheduleFile = (
            self.dirEtc + "config-source" + str(currentSource) + "-schedule.json"
        )
        self.log.info(
            "reportsDaily.generateReport(): Looking for Schedule file: %(scheduleFile)s"
            % {"scheduleFile": str(scheduleFile)}
        )
    ...
    def writeJsonFile(self, jsonFile, jsonContent):
        ...
        """
        self.log.debug("reportsDaily.writeJsonFile(): Start")
    ...
    def getFileSystemUsage(self, directory):
        ...
        self.log.debug("reportsDaily.getFileSystemUsage(): Start")
        statvfs = os.statvfs(directory)
        totalSpace = statvfs.f_frsize * statvfs.f_blocks  # Size of filesystem in bytes
        totalFreeSpace = (
            statvfs.f_frsize * statvfs.f_bavail
        )  # Number of free bytes that ordinary user
        return {
            "free": totalFreeSpace,
            "total": totalSpace,
            "used": int(totalSpace - totalFreeSpace),
            "percentUsed": int(totalFreeSpace / totalSpace * 100),
        }
