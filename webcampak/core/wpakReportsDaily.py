...
class reportsDaily(object):
    ...
    def initGetText(self, dirLocale, cfgsystemlang, cfggettextdomain):
        ...
        try:
            ...
            _ = t.ugettext
            t.install()
            self.log.info("reportsDaily.initGetText(): " + _(
    ...
    def run(self):
        ...
        for currentSource in self.sourcesUtils.getActiveSourcesIds():
            self.log.info("reportsDaily.run(): " + _("Processing source %(currentSource)s") % {
            ...
            self.log.info(
                "reportsDaily.run(): " + _("Source disk quota is: %(sourceQuota)s") % {'sourceQuota': str(sourceQuota)})
            ...
            for currentReportDay in missingReports:
                if int(self.timeUtils.getCurrentDate().strftime("%Y%m%d")) == currentReportDay:
                    self.log.info(
                        "reportsDaily.run(): " + _("Skipping report for current day: %(currentReportDay)s") % {
                    ...
                    currentSourceReport['source'] = {'quota': sourceQuota, 'usage': sourceDiskUsage,
                                                     'percentUsed': sourceDiskPercentUsed}
                    currentSourceReport['disk'] = self.getFileSystemUsage(self.dirSources)
                    dirCurrentSourceReports = self.dirSources + 'source' + str(currentSource) + '/' + \
                    ...
                    self.log.info("reportsDaily.run(): " + _("Preparing to save report in %(jsonReportFile)s") % {
                        'jsonReportFile': str(jsonReportFile)})
                    if self.writeJsonFile(jsonReportFile, currentSourceReport):
                        reportEmail = self.prepareEmailReportContent(currentSource, currentReportDay,
                                                                     currentSourceReport)
                        emailReports[currentSource][currentReportDay] = {'sourceid': currentSource,
                                                                         'reportDay': currentReportDay,
                                                                         'report': currentSourceReport,
                                                                         'reportEmail': reportEmail}
                    else:
                        self.log.error(
                            "reportsDaily.run(): " + _("Error saving report file to disk %(jsonReportFile)s") % {
                                'jsonReportFile': str(jsonReportFile)})
    ...
    def sendReportEmail(self, currentUser, emailReports):
        ...
        self.log.info(
            "reportsDaily.sendReportEmail(): " + _("Preparing to send email to user %(name)s - email: %(email)s") % {
                'name': currentUser['name'], 'email': currentUser['email']})

        emailReportBody = ""
        currentSourceId = None
        for currentSource in currentUser['sources']:
            self.log.info("reportsDaily.sendReportEmail(): " + _("Looking for report for source %(sourceid)s") % {
            ...
            if currentSource['sourceid'] in emailReports:
                for currentDailyReport in sorted(emailReports[currentSource['sourceid']]):
                    ...
                    emailReportBody = emailReportBody + emailReports[currentSource['sourceid']][currentDailyReport][
                        'reportEmail'] + "\n -----------------------\n"
            else:
                self.log.info("reportsDaily.sendReportEmail(): " + _("No report found for source %(sourceid)s") % {
                    'sourceid': currentSource['sourceid']})
        ...
        if currentSourceId != None:
            ...
            dirCurrentLocaleEmails = self.dirLocale + configSource.getConfig(
                'cfgsourcelanguage') + "/" + self.dirLocaleEmails
            ...
            self.log.info("captureportsDailyre.sendReportEmail(): " + _(
                "Using message subject file: %(emailReportSubjectFile)s") % {
                              'emailReportSubjectFile': emailReportSubjectFile})
            self.log.info(
                "reportsDaily.sendReportEmail(): " + _("Using message content file: %(emailReportContentFile)s") % {
            ...
            emailReportSubject = emailReportSubject.replace("#REPORTDATE#",
                                                            self.timeUtils.getCurrentDate().strftime("%B %d, %Y"))
            self.log.info("reportsDaily.sendReportEmail(): " + _("Email subject: %(emailReportSubject)s") % {
            ...
            self.log.info("reportsDaily.sendReportEmail(): " + _("Email Report content: %(emailReportContent)s") % {
                'emailReportContent': "\n" + emailReportContent})
    ...
    def prepareEmailReportContent(self, currentSource, currentReportDay, currentSourceReport):
        ...
        self.log.info("reportsDaily.prepareEmailReportContent(): " + _(
            "Preparing report content for Source: %(currentSource)s and Day: %(currentReportDay)s") % {
        ...
        table = [ \
            ["Source", fileUtils.sizeof_fmt(currentSourceReport['source']['usage']),
             fileUtils.sizeof_fmt(currentSourceReport['source']['quota']), fileUtils.sizeof_fmt(currentSourceAvailable),
        ...
        if currentSourceReport['schedule'] != None:
            jpgScore = currentSourceReport['schedule']['onschedule']['JPG']['count'] / (
            currentSourceReport['schedule']['onschedule']['JPG']['count'] +
            currentSourceReport['schedule']['missing']['JPG']['count']) * 100
            rawScore = currentSourceReport['schedule']['onschedule']['RAW']['count'] / (
            currentSourceReport['schedule']['onschedule']['RAW']['count'] +
            currentSourceReport['schedule']['missing']['RAW']['count']) * 100
    ...
    def generateReport(self, currentSource, currentReportDay, sourceSchedule):
        ...
        self.log.info("reportsDaily.generateReport(): " + _(
            "Generating report for Source: %(currentSource)s and Day: %(currentReportDay)s") % {
                          'currentSource': str(currentSource), 'currentReportDay': str(currentReportDay)})

        dirCurrentSourcePictures = self.dirSources + 'source' + str(currentSource) + '/' + \
    ...
    def compareScheduleWithDisk(self, sourceSchedule, currentJPGs, currentRAWs, reportDatetime):
        ...
        for hour in range(24):
            for minute in range(59):
                ...
                self.log.debug(
                    "reportsDaily.compareScheduleWithDisk(): " + _("Testing timeslot: %(fullHour)s:%(fullMinute)s") % {
                ...
                if reportDayOfWeek in sourceSchedule:
                    ...
                    if testIdx in currentJPGs['list']:
                        if fullHour in sourceSchedule[reportDayOfWeek] and fullMinute in \
                                sourceSchedule[reportDayOfWeek][fullHour] and sourceSchedule[reportDayOfWeek][fullHour][
                            fullMinute] == 'Y':
        ...
        return { \
            'onschedule': {'JPG': {'list': intersectJPG, 'count': len(intersectJPG)},
                           'RAW': {'list': intersectRAW, 'count': len(intersectRAW)}} \
            , 'missing': {'JPG': {'list': missingJPG, 'count': len(missingJPG)},
                          'RAW': {'list': missingRAW, 'count': len(missingRAW)}} \
            , 'extra': {'JPG': {'list': extraJPG, 'count': len(extraJPG)},
    ...
    def getPicturesStats(self, picturesDirectory):
        ...
        self.log.info("reportsDaily.getPicturesStats(): " + _("Scanning directory: %(picturesDirectory)s") % {
        ...
        if os.path.exists(picturesDirectory):
            for currentPicture in os.listdir(picturesDirectory):
                if len(os.path.splitext(os.path.basename(currentPicture))[0]) == 14 and (
                        os.path.splitext(os.path.basename(currentPicture))[1] == ".jpg" or
                        os.path.splitext(os.path.basename(currentPicture))[1] == ".raw"):
    ...
    def getMissingReports(self, sourceId):
        ...
        dirCurrentSourcePictures = self.dirSources + 'source' + str(sourceId) + '/' + \
                                   self.configPaths.getConfig('parameters')['dir_source_pictures']
        dirCurrentSourceReports = self.dirSources + 'source' + str(sourceId) + '/' + \
                                  self.configPaths.getConfig('parameters')['dir_source_resources_reports']
        missingDays = []
        for picturesDay in os.listdir(dirCurrentSourcePictures):
            if picturesDay[0:2] == "20" and len(picturesDay) == 8:
                if os.path.isfile(dirCurrentSourceReports + picturesDay + '.json'):
                    self.log.info("reportsDaily.getMissingReports(): " + _("Report exists for day: %(picturesDay)s") % {
                        'picturesDay': str(picturesDay)})
                else:
                    self.log.info(
                        "reportsDaily.getMissingReports(): " + _("Report does not exist for day: %(picturesDay)s") % {
                            'picturesDay': str(picturesDay)})
    ...
    def getSourceSchedule(self, currentSource):
        ...
        self.log.info("reportsDaily.generateReport(): " + _("Looking for Schedule file: %(scheduleFile)s") % {
    ...
    def getFileSystemUsage(self, directory):
        ...
        return {'free': totalFreeSpace, 'total': totalSpace, 'used': int(totalSpace - totalFreeSpace),
