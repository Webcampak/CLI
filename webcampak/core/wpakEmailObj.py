...
class emailObj(object):
    ...
    def __init__(self, log, dirEmails, fileUtils):
        ...
        self.log.info("emailObj(): " + _("Setting default filename to: %(emailObjFile)s") % {'emailObjFile': self.emailObjFile} )
                        ...
                        , "ATTACHMENTS": {
                            ...
                            , "items": {
                                ...
                                , "properties": {
                                    ...
                                    , "WIDTH":  {"type": "number", "description": "Width of the attachment, height will be calculated automatically"}
                                }
                            }                                        
                        }            
                    }
    ...
    def setEmailObjectFile(self, emailObjFile):
        self.log.info("emailObj.setEmailObjectFile(): " + _("Email Object file set to: %(emailObjFile)s") % {'emailObjFile': emailObjFile})            
    ...
    def writeEmailObjectFile(self):
        ...
        if self.writeJsonFile(self.emailObjFile, self.getEmailObject()) == True:
            self.log.info("emailObj.writeEmailObjectFile(): " + _("Successfully added email to queue, file: %(emailObjFile)s") % {'emailObjFile': str(self.emailObjFile)})
            return True
        else:
    ...
    def loadJsonFile(self, jsonFile):
        ...
        if os.path.isfile(jsonFile):
            self.log.info("emailObj.loadJsonFile(): " + _("Load JSON file into memory: %(jsonFile)s") % {'jsonFile': jsonFile} )                    
