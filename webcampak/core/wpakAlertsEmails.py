...
class alertsEmails(object):
    ...
    def loadEmailTemplateFile(self, configSource, TemplateFilename):
        ...
        self.log.debug("alertsEmails.loadEmailTemplateFile(): Start")
        ...
        if os.path.isfile(templateFile) == False:
            templateFile = (
                self.dirLocale + "en_US.utf8/" + self.dirLocaleEmails + TemplateFilename
            )
        if os.path.isfile(templateFile):
            self.log.info(
                "alertsEmails.sendCaptureSuccess(): Using message subject file: %(templateFile)s"
    ...
    def sendCaptureError(self, currentAlert):
        ...
        Returns:
            None
        """
        self.log.debug("alertsEmails.sendCaptureSuccess(): Start")
        configSource = self.configCache.getSourceConfig(
            "source", currentAlert.getAlertValue("sourceid")
        )
        ...
        if currentAlert.getAlertValue("emailType") == "REMINDER":
            emailSubject = self.loadEmailTemplateFile(
                configSource, "alertErrorReminderSubject.txt"
            )
        else:
            emailSubject = self.loadEmailTemplateFile(
                configSource, "alertErrorSubject.txt"
            )

        if emailContent != None and emailSubject != None:
            currentSourceTime = dateutil.parser.parse(
                currentAlert.getAlertValue("currentTime")
            )
            lastCatpureTime = dateutil.parser.parse(
            ...
            emailSubject = emailSubject.replace(
                "#CURRENTSOURCE#", str(currentAlert.getAlertValue("sourceid"))
            )
            emailSubject = emailSubject.replace(
                "#CURRENTSOURCENAME#",
                self.dbUtils.getSourceName(currentAlert.getAlertValue("sourceid")),
            )
            emailSubject = emailSubject.replace(
                "#LASTCAPTURETIME#", lastCatpureTime.strftime("%c")
            )
            emailContent = emailContent.replace(
                "#CURRENTSOURCETIME#", currentSourceTime.strftime("%c")
            )
            emailContent = emailContent.replace(
                "#LASTCAPTURETIME#", lastCatpureTime.strftime("%c")
            )
            ...
            newEmail.field_to = self.dbUtils.getSourceEmailUsers(
                currentAlert.getAlertValue("sourceid")
            )
            ...
            self.log.info(
                "alertsEmails.sendCaptureSuccess(): Unable to find default translation files to be used"
            )
    ...
    def sendCaptureSuccess(self, currentAlert):
        ...
        emailContent = self.loadEmailTemplateFile(
            configSource, "alertWorkingContent.txt"
        )
        emailSubject = self.loadEmailTemplateFile(
            configSource, "alertWorkingSubject.txt"
        )
        ...
        if emailContent != None and emailSubject != None:
            ...
            newEmail.subject = emailSubject
            if (
                currentAlert.getAlertValue("lastPicture") != None
                and int(configSource.getConfig("cfgemailsuccesspicturewidth")) > 0
            ):
                captureDirectory = currentAlert.getAlertValue("lastPicture")[:8]
                dirCurrentSourcePictures = (
                    self.dirSources
                    + "source"
                    + str(currentAlert.getAlertValue("sourceid"))
                    + "/"
                ...
                if os.path.isfile(
                    ...
                    + captureDirectory
                    + "/"
                    + currentAlert.getAlertValue("lastPicture")
                ):
                    captureFilename = (
                        dirCurrentSourcePictures
                        + captureDirectory
                    ...
                    newEmail.attachments.append(
                        {
                            ...
                            "WIDTH": int(
                                configSource.getConfig("cfgemailsuccesspicturewidth")
                            ),
