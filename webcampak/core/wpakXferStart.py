...
class xferStart(object):
    def __init__(self, log, appConfig, config_dir, threadUUID):
        ...
        self.dirSyncReports = self.configPaths.getConfig("parameters")[
            "dir_sync-reports"
        ]
        ...
        if self.getArgThreadUUID() == None:
            ...
            for currentThreadUUID in self.xferUtils.getThreadsUUID():
                ...
                if self.xferUtils.isThreadRunning(currentThreadUUID) == False:
                    p[currentThreadUUID] = Process(
                        target=self.startThread, args=(currentThreadUUID,)
                    )
                    p[currentThreadUUID].start()
                    self.xferUtils.setThreadPid(
                        currentThreadUUID, p[currentThreadUUID].pid
                    )
                else:
                    self.log.info(
                        "xferStart.run(): Thread already running: " + currentThreadUUID
                    )
    ...
    def startThread(self, threadUUID):
        self.log.info(
            "("
            + str(os.getpid())
            + ")xferStart.startThread(): Processing Thread UUID: "
            + threadUUID
        )
        self.log.info(
            "("
            + str(os.getpid())
            + ")xferStart.startThread(): Thread process: "
            + str(os.getpid())
        )
        # Load the config containing all paths and the general config file
        if self.xferUtils.checkThreadUUID(threadUUID):
            ...
            if currentThreadQueueCount == 0:
                self.log.info(
                    "("
                    + str(os.getpid())
                    + ")xferStart.startThread(): Thread is empty leaving process"
                )
            else:
                self.log.info(
                    "("
                    + str(os.getpid())
                    + ")xferStart.startThread(): Number of files left in the thread: "
                    + str(currentThreadQueueCount)
                )
                ...
                if firstThreadFile != None:
                    self.log.info(
                        "("
                        + str(os.getpid())
                        + ")xferStart.startThread(): Processing File: "
                        + firstThreadFile
                    )
            ...
            self.log.info(
                "("
                + str(os.getpid())
                + ")xferStart.startThread(): Thread UUID does not exist: "
                + threadUUID
            )
    ...
    def processJob(self, threadUUID, firstThreadFile):
        ...
        jobJsonContent = self.xferUtils.logToJson(
            firstThreadFile,
            jobJsonContent,
            "Checking file size on source: " + str(jobSourceFilesize) + " bytes",
        )
        jobDestinationFilesize = self.getJobFilesize(
            jobJsonContent["job"]["destination"]
        )
        jobJsonContent = self.xferUtils.logToJson(
            firstThreadFile,
            jobJsonContent,
            "Checking file size on destination: "
            + str(jobDestinationFilesize)
            + " bytes",
        )
        ...
        if jobSourceFilesize != jobDestinationFilesize:
            self.log.info(
                "("
                + str(os.getpid())
                + ")xferStart.processJob(): Source and destination are different, starting process"
            )
            jobJsonContent = self.xferUtils.logToJson(
                firstThreadFile,
                jobJsonContent,
            ...
            if "retries" in jobJsonContent["job"]:
                jobJsonContent["job"]["retries"] = (
                    int(jobJsonContent["job"]["retries"]) - 1
                )
            else:
            ...
            self.log.info(
                "("
                + str(os.getpid())
                + ")xferStart.processJob(): Current retries count: "
                + str(jobJsonContent["job"]["retries"])
            )
            ...
            if (
                jobJsonContent["job"]["source"]["type"] == "filesystem"
                and jobJsonContent["job"]["destination"]["type"] == "ftp"
            ):
                self.log.info(
                    "("
                    + str(os.getpid())
                    + ")xferStart.processJob(): Copy from local source to remote FTP destination"
                )
                jobJsonContent["job"]["filesourceid"] = jobJsonContent["job"]["source"][
                    "sourceid"
                ]
                jobJsonContent = self.xferUtils.logToJson(
                    firstThreadFile,
                ...
                jobJsonContent = self.processFTPFile(
                    firstThreadFile,
                    jobJsonContent,
                    "destination",
                    "source",
                ...
                self.xferUtils.setThreadLastJob(
                    threadUUID, jobJsonContent["job"]["xfer_report"]
                )
                self.moveThreadFileAfterTransfer(jobJsonContent, firstThreadFile)

            elif (
                jobJsonContent["job"]["source"]["type"] == "ftp"
                and jobJsonContent["job"]["destination"]["type"] == "filesystem"
            ):
                self.log.info(
                    "("
                    + str(os.getpid())
                    + ")xferStart.processJob(): Will copy from remote FTP source to local filesystem"
                )
                jobJsonContent["job"]["filesourceid"] = jobJsonContent["job"][
                    "destination"
                ]["sourceid"]
                jobJsonContent = self.xferUtils.logToJson(
                    firstThreadFile,
            ...
            self.log.info(
                "("
                + str(os.getpid())
                + ")xferStart.processJob(): Error, file does not exist on source, verify your configuration"
            )
            ...
            self.log.info(
                "("
                + str(os.getpid())
                + ")xferStart.processJob(): File already exists, not copying anything"
            )
            os.remove(firstThreadFile)
    ...
    def moveThreadFileAfterTransfer(self, jobJsonContent, firstThreadFile):
        self.log.info(
            "("
            + str(os.getpid())
            + ")xferStart.moveThreadFileAfterTransfer(): Filepath: "
            + firstThreadFile
        )
        if jobJsonContent["job"]["xfer_report"] != None:
            self.log.info(
                "("
                + str(os.getpid())
                + ")xferStart.moveThreadFileAfterTransfer(): Transfer has been successful, storing completed file"
            )
            if "sync-report" in jobJsonContent["job"]:
                ...
                destinationFilePath = (
                    ...
                    + "/completed/"
                    + os.path.splitext(
                        jobJsonContent["job"]["sync-report"]["filename"]
                    )[0]
                    + "/"
                )
            else:
                destinationFilePath = (
                    self.dirSources
                    + "/source"
                    + str(jobJsonContent["job"]["filesourceid"])
                    + "/resources/xfer/"
                    + os.path.basename(firstThreadFile)[0:8]
                    + "/"
                )
            self.log.info(
                "("
                + str(os.getpid())
                + ")xferStart.moveThreadFileAfterTransfer(): Storing to: "
                + destinationFilePath
            )
            destinationJobFile = fileUtils.CheckDir(destinationFilePath)
            self.xferUtils.writeJsonFileGzip(
                destinationFilePath + os.path.basename(firstThreadFile) + ".gz",
                jobJsonContent,
            )
            self.log.info(
                "("
                + str(os.getpid())
                + ")xferStart.moveThreadFileAfterTransfer(): Removing file: "
                + firstThreadFile
            )
            os.remove(firstThreadFile)
            ...
            and jobJsonContent["job"]["retries"] >= 0
        ):
            self.log.info(
                "("
                + str(os.getpid())
                + ")xferStart.moveThreadFileAfterTransfer(): Transfer failed, moving file back to queue"
            )
            fileUtils.CheckDir(
                self.dirXferQueue + os.path.basename(firstThreadFile)[0:8]
            )
            os.rename(
                firstThreadFile,
                self.dirXferQueue
                + os.path.basename(firstThreadFile)[0:8]
                + "/"
                + os.path.basename(firstThreadFile),
            )
            self.log.info(
            ...
            self.log.info(
                "("
                + str(os.getpid())
                + ")xferStart.moveThreadFileAfterTransfer(): Transfer failed and exceeded retries, moving file to failed directory"
            )
            fileUtils.CheckDir(
                self.dirXferFailed + os.path.basename(firstThreadFile)[0:8]
            )
            self.xferUtils.writeJsonFileGzip(
                self.dirXferFailed
                + os.path.basename(firstThreadFile)[0:8]
            ...
            self.log.info(
                "("
                + str(os.getpid())
                + ")xferStart.moveThreadFileAfterTransfer(): Target filename for failed file: "
                + self.dirXferFailed
                + os.path.basename(firstThreadFile)[0:8]
                + "/"
                + os.path.basename(firstThreadFile)
    ...
    def processFTPFile(
        ...
        jobJsonContent = self.xferUtils.logToJson(
            firstThreadFile, jobJsonContent, "Establishing FTP connection"
        )
        currentFTPConnectionStatus = currentFTP.initByServerId(
            jobJsonContent["job"][ftpFileLocation]["sourceid"],
            jobJsonContent["job"][ftpFileLocation]["ftpserverid"],
        )
        localFilepath = (
            self.dirSources
            + "source"
            + str(jobJsonContent["job"][localFileLocation]["sourceid"])
            + "/"
        ...
        ftpServerConfig = Config(
            ...
            self.dirEtc
            + "config-source"
            + str(jobJsonContent["job"][ftpFileLocation]["sourceid"])
            + "-ftpservers.cfg",
        )
        remoteFilepath = (
            ftpServerConfig.getConfig(
                "cfgftpserverslist"
                + str(jobJsonContent["job"][ftpFileLocation]["ftpserverid"])
            )[4]
        ...
        if ftpTransferSuccess:
            ...
            jobJsonContent = self.xferUtils.logToJson(
                firstThreadFile, jobJsonContent, "Unable to upload file"
            )
            jobJsonContent["job"]["xfer_report"] = {}
        currentFTP.closeFtp()
    ...
    def getJobFilesize(self, job):
        ...
        if job["type"] == "filesystem":
            ...
            self.log.info(
                "("
                + str(os.getpid())
                + ")xferStart.getJobFilesize(): Filesystem path: "
                + filesystemPath
            )
            ...
            ftpFilepath = (
                ftpServerConfig.getConfig(
                    "cfgftpserverslist" + str(job["ftpserverid"])
                )[4]
                + job["filepath"]
            )
            self.log.info(
                "("
                + str(os.getpid())
                + ")xferStart.getJobFilesize(): FTP path: "
                + ftpFilepath
            )
            currentFTP = FTPTransfer(self.log, self.config_dir)
            currentFTPConnectionStatus = currentFTP.initByServerId(
                job["sourceid"], job["ftpserverid"]
            )
