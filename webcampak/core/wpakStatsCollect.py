        ...
        for scanfile in sorted(os.listdir(self.dirEtc), reverse=True):
            if re.findall('config-source[0-9]+.cfg', scanfile):
                ...
                if os.path.isdir(self.dirSources + "source" + sourceid + "/"):
                    ...
                    fileUtils.CheckFilepath(
                        self.dirSources + "source" + sourceid + "/resources/stats/" + cfgcurrentday + ".txt")
                    sourceStats = OrderedDict()
                    sourceStats['date'] = cfgnow.isoformat()
                    StatsFile = Config(self.log,
                                       self.dirSources + "source" + sourceid + "/resources/stats/" + cfgcurrentday + ".txt")
                    if os.path.isdir(self.dirSources + "source" + sourceid + "/pictures/"):
                        StatsFile.setSensor('PicDirScan', "", "")
                        daysStats = OrderedDict()
                        for listpictdir in sorted(os.listdir(self.dirSources + "source" + sourceid + "/pictures/"),
                                                  reverse=False):
                            if listpictdir[:2] == "20" and os.path.isdir(
                                                                    self.dirSources + "source" + sourceid + "/pictures/" + listpictdir):
                                self.log.info(
                                    "statsCollect.run(): Scanning directory:" + self.dirSources + "source" + sourceid + "/pictures/" + listpictdir)
                                daysStats[listpictdir] = {'count': len(glob.glob(
                                    self.dirSources + "source" + sourceid + "/pictures/" + listpictdir + "/*.jpg")),
                                                          'size': fileUtils.CheckDirDu(
                                                              self.dirSources + "source" + sourceid + "/pictures/" + listpictdir + "/")}
                                StatsFile.setSensor('PicDirScan', 'ScannedDay' + listpictdir, [len(glob.glob(
                                    self.dirSources + "source" + sourceid + "/pictures/" + listpictdir + "/*.jpg")),
                                                                                               fileUtils.CheckDirDu(
                                                                                                   self.dirSources + "source" + sourceid + "/pictures/" + listpictdir + "/")])
                        sourceStats['days'] = daysStats
                    StatsFile.setSensor(cfgcurrentdaytime, "", "")
                    StatsFile.setSensor(cfgcurrentdaytime, 'Timestamp', cfgnow.strftime("%s"))
                    if os.path.isdir(self.dirSources + "source" + sourceid + "/pictures/"):
                        sourceStats['PicturesSize'] = fileUtils.CheckDirDu(
                            self.dirSources + "source" + sourceid + "/pictures/")
                    ...
                    if os.path.isdir(self.dirSources + "source" + sourceid + "/videos/"):
                        sourceStats['VideoSize'] = fileUtils.CheckDirDu(
                            self.dirSources + "source" + sourceid + "/videos/")
                    ...
                    with open(self.dirSources + "source" + sourceid + "/resources/stats/" + cfgcurrentday + ".jsonl",
                              "a") as sourceStatsFile:
