...
class xferStop(object):
    ...
    def stopThread(self, threadUUID):
        self.log.info(
            "("
            + str(os.getpid())
            + ")xferStop.stopThread(): Processing Thread UUID: "
            + threadUUID
        )
        ...
        if self.xferUtils.isPidAlive(threadPid):
            self.log.info(
                "("
                + str(os.getpid())
                + ")xferStop.stopThread(): Thread is currently alive, issuing SIGKILL"
            )
            ...
            self.log.info(
                "("
                + str(os.getpid())
                + ")xferStop.stopThread(): Thread is not alive, doing nothing"
            )
