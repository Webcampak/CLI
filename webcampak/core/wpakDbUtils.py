...
class dbUtils(object):
    ...
    def getSourceEmailUsers(self, sourceId):
        self.log.debug("dbUtils.getUsers(): Start")
    ...
    def getUsersWithSourceAlerts(self):
        self.log.debug("dbUtils.getUsersWithSourceAlerts(): Start")
        if self.dbConnection == None:
            self.openDb()
    ...
    def getUsersAlertsForSource(self, sourceId):
        self.log.debug("dbUtils.getUsersAlertsForSource(): Start")
        if self.dbConnection == None:
            self.openDb()
        ...
        for row in self.dbCursor.fetchall():
            useId, email, firstname, lastname = row
            users.append(
                {"useId": useId, "name": firstname + " " + lastname, "email": email}
            )
        self.closeDb()
        return users

    def getSourcesForUser(self, useId):
        self.log.debug("dbUtils.getSourcesForUser(): Start")
        if self.dbConnection == None:
            self.openDb()
        ...
        for row in self.dbCursor.fetchall():
            ...
            sources.append(
                {"sourceid": sourceid, "name": name, "alertsFlag": alertsFlag}
            )
        self.closeDb()
        return sources
    ...
    def getSourceName(self, sourceId):
        self.log.debug("dbUtils.getSourceName(): Start")
        if self.dbConnection == None:
            self.openDb()
