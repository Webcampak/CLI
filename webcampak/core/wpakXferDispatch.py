...
class xferDispatch(object):
    ...
    def run(self):
        ...
        self.log.info(
            "xferDispatch.run(): Maximum number of threads for the queue: "
            + self.xferUtils.getCfgxferthreads()
        )
    ...
    def populateThreads(self):
        ...
        allQueuedFiles = (
            self.xferUtils.getAllQueuedFiles()
        )  # List all files currently queued
        ...
        for currentQueuedFile in allQueuedFiles:
            ...
            if self.xferUtils.areThreadsFull():
                ...
                try:
                    ...
                    self.log.error(
                        "xferDispatch.run(): Unable to JSON decode: "
                        + currentQueuedFile
                    )
                    fileUtils.CheckDir(self.dirXferFailed)
                    shutil.move(
                        currentQueuedFile,
                        self.dirXferFailed + os.path.basename(currentQueuedFile),
                    )
                    break

                if queuedJson["job"]["source"]["type"] == "ftp":
                    ftpserverHash = queuedJson["job"]["source"]["ftpserverhash"]
                    ftpserverMaxThreads = self.getMaxThreadsForFTPServer(
                        self.dirEtc
                        + "config-source"
                        + str(queuedJson["job"]["source"]["sourceid"])
                        + "-ftpservers.cfg",
                        str(queuedJson["job"]["source"]["ftpserverid"]),
                    )
                    ...
                    ftpserverMaxThreads = self.getMaxThreadsForFTPServer(
                        self.dirEtc
                        + "config-source"
                        + str(queuedJson["job"]["destination"]["sourceid"])
                        + "-ftpservers.cfg",
                        str(queuedJson["job"]["destination"]["ftpserverid"]),
                    )

                if ftpserverHash != None:
                    self.log.info(
                        "xferDispatch.populateThreads(): FTP Server Hash: "
                        + ftpserverHash
                    )
                    self.log.info(
                        "xferDispatch.populateThreads(): FTP Server Threads: "
                        + ftpserverMaxThreads
                    )
                    if ftpserverHash in fullServers:
                        ...
                        targetThread = self.identifyTargetThread(
                            threadStats, int(ftpserverMaxThreads)
                        )
                        if targetThread == None:
                            ...
                            self.log.info(
                                "xferDispatch.populateThreads(): Move file to: "
                                + self.dirXferThreads
                                + targetThread
                                + "/"
                                + os.path.basename(currentQueuedFile)
                            )
                            shutil.move(
                                currentQueuedFile,
                                self.dirXferThreads
    ...
    def identifyTargetThread(self, threadStats, ftpserverMaxThreads):
        ...
        self.log.info(
            "xferDispatch.identifyTargetThread(): FTP Server Max Threads: "
            + str(ftpserverMaxThreads)
        )
        ...
        removedFullThreads = {}
        for key in sorted(
            threadStats, key=lambda x: threadStats[x]["filesCount"], reverse=True
        ):
            ...
            if threadStats[key]["filesCount"] < self.xferUtils.getMaxFilesPerThread():
                ...
                threadStats[key]["filesCount"] >= self.xferUtils.getMaxFilesPerThread()
                and threadStats[key]["hashCount"] > 0
            ):
                removedFullThreads[key] = threadStats[key]
                # else:
                #    print 'skipping...'
        ...
        for key in sorted(
            removedFullThreads,
            key=lambda x: removedFullThreads[x]["filesCount"],
            reverse=True,
        ):
            if removedFullThreads[key]["hashCount"] > 0:
                nbActiveThreads = nbActiveThreads + 1
        self.log.info(
            "xferDispatch.identifyTargetThread(): Number of available threads with this server Hash: "
            + str(nbActiveThreads)
        )
        ...
        if nbActiveThreads < ftpserverMaxThreads:
            ...
            for key in sorted(
                removedFullThreads, key=lambda x: removedFullThreads[x]["filesCount"]
            )[:1]:
                if removedFullThreads[key]["hashCount"] == 0:
            ...
            for key in sorted(
                ...
                if (
                    removedFullThreads[key]["hashCount"] > 0
                    and removedFullThreads[key]["filesCount"]
                    < self.xferUtils.getMaxFilesPerThread()
                ):
                    sortedFilesThreads[key] = removedFullThreads[key]

                    # 3b - From the resulting list, get the thread with the lowest amount of files
            for key in sorted(
                sortedFilesThreads, key=lambda x: sortedFilesThreads[x]["filesCount"]
            )[:1]:
                # print '3: ', key, sortedFilesThreads[key]
                identifiedThread = key

        self.log.info(
            "xferDispatch.identifyTargetThread(): Identified thread for queued file: "
            + str(identifiedThread)
        )
    ...
    def getMaxThreadsForFTPServer(self, configFile, serverId):
        ...
        self.log.info(
            "xferDispatch.getMaxThreadsForFTPServer(): Config File: " + configFile
        )
        self.log.info(
            "xferDispatch.getMaxThreadsForFTPServer(): Server ID: " + serverId
        )
    ...
    def initializeThreads(self, threadsNumber):
        ...
        self.log.info(
            "xferDispatch.initializeThreads(): Threads Dir: " + self.dirXferThreads
        )
        self.log.info(
            "xferDispatch.initializeThreads(): Threads Number: " + str(threadsNumber)
        )
        ...
        for currentThreadUUID in self.xferUtils.getThreadsUUID():
            self.log.info(
                "xferDispatch.initializeThreads(): Current thread file: "
                + currentThreadUUID
                + ".json"
            )
            if os.path.exists(self.dirXferThreads + currentThreadUUID):
                threadJson = self.xferUtils.loadJsonFile(
                    self.dirXferThreads + currentThreadUUID + ".json"
                )
                if (
                    threadJson != {}
                    and "last_job" in threadJson
                    and threadJson["last_job"] != None
                ):
                    lastJobCompletion = dateutil.parser.parse(
                        threadJson["last_job"]["date_completed"]
                    )
                    ...
                    if (currentDate - lastJobCompletion).total_seconds() < 1800:
                        ...
                        if "pid" in threadJson and self.xferUtils.isPidAlive(
                            threadJson["pid"]
                        ):
                            self.log.info(
                                "xferDispatch.initializeThreads(): Thread is alive, PID: "
                                + str(threadJson["pid"])
                            )
                            threadsCpt = threadsCpt + 1
                        else:
                            self.log.info(
                                "xferDispatch.initializeThreads(): This thread is not running"
                            )
                            self.clearThreadDirectory(
                                self.dirXferThreads + currentThreadUUID
                            )
                    else:
                        self.log.info(
                        ...
                        self.log.info(
                            "xferDispatch.initializeThreads(): Killing the process"
                        )
                ...
                self.log.info(
                    "xferDispatch.initializeThreads(): Associated directory does not exist: "
                    + self.dirXferThreads
                    + currentThreadUUID
                )
                self.log.info(
                    "xferDispatch.initializeThreads(): Removing: "
                    + self.dirXferThreads
                    + currentThreadUUID
                )
        ...
        self.log.info(
            "xferDispatch.initializeThreads(): Number of threads to create: "
            + str(nbThreadsToCreate)
        )
        for i in range(0, nbThreadsToCreate):
            threadUuid = uuid.uuid4()
            self.log.info(
                "xferDispatch.initializeThreads(): Creating thread #"
                + str(i)
                + " UUID: "
                + str(threadUuid)
            )
            ...
            if self.xferUtils.writeJsonFile(
                self.dirXferThreads + str(threadUuid) + ".json", threadJson
            ):
    ...
    def clearThreadDirectory(self, currentThreadDirectory):
        ...
        for currentFilename in [
            f for f in os.listdir(currentThreadDirectory) if f.endswith(".json")
        ]:
            fileUtils.CheckDir(self.dirXferQueue + currentFilename[0:8])
            os.rename(
                currentThreadDirectory + "/" + currentFilename,
                self.dirXferQueue + currentFilename[0:8] + "/" + currentFilename,
            )
            self.log.info(
                "xferDispatch.clearThreadDirectory(): Moved: "
                + currentFilename
                + " to: "
                + self.dirXferQueue
                + currentFilename[0:8]
    ...
    def getThreadStats(self, ftpserverHash):
        ...
        threads = {}
        for currentFilename in [
            f for f in os.listdir(self.dirXferThreads) if f.endswith(".json")
        ]:
            currentThreadHash = os.path.splitext(currentFilename)[0]
            if os.path.isdir(self.dirXferThreads + currentThreadHash + "/"):
                self.log.info(
                    "xferDispatch.getThreadStats(): Calculating Thread: "
                    + currentThreadHash
                )
                threadsFilesCount = 0
                ...
                for currentFilename in [
                    f
                    for f in os.listdir(self.dirXferThreads + currentThreadHash + "/")
                    if f.endswith(".json")
                ]:
                    try:
                        threadJson = self.xferUtils.loadJsonFile(
                        ...
                        if (
                            threadJson["job"]["destination"]["ftpserverhash"]
                            == ftpserverHash
                            or threadJson["job"]["source"]["ftpserverhash"]
                            == ftpserverHash
                        ):
                            ftpserverHashCount = ftpserverHashCount + 1
                    except:
                        self.log.info(
                            "timeUtils.getTimeFromExif(): File was likely removed during processing: %(filePath)s"
                            % {
                                "filePath": self.dirXferThreads
                                + currentThreadHash
                                + "/"
                                + currentFilename
                            }
                ...
                self.log.info(
                    "xferDispatch.getThreadStats(): Thread: "
                    + currentThreadHash
                    + " Files Count: "
                    + str(threadsFilesCount)
                    + " Hash Count: "
                    + str(ftpserverHashCount)
                )
        return threads
