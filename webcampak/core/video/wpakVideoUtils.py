...
#				keepend = int(self.configSourceVideo.getConfig('cfgcustomkeependhour') + self.configSourceVideo.getConfig('cfgcustomkeependminute'))
    ...
    def compareImages(self, currentFile):
        ...
        if self.configSourceVideo.getConfig('cfgfilterwatermarkfile') != "":
            watermarkFile = None
            if os.path.isfile(self.dirCurrentSourceWatermarkDir + self.configSourceVideo.getConfig('cfgfilterwatermarkfile')):
                watermarkFile = self.dirCurrentSourceWatermarkDir + self.configSourceVideo.getConfig('cfgfilterwatermarkfile')
    ...
    def copyFilesToVideoDirectory(self):
        ...
        for scanPictureDay in sorted(os.listdir(self.dirCurrentSourcePictures), reverse=True):	
            if scanPictureDay[:2] == "20" and os.path.isdir(self.dirCurrentSourcePictures + scanPictureDay): 
                ...
                if scanPictureDay[:8] == self.videoClass.getScriptStartTime().strftime("%Y%m%d") and self.videoType == "video" and VideoTag == False:
                    self.log.info("videoUtils.copyFilesToVideoDirectory(): " + _("Date: %(currentDay)s: Error, you are trying to create a daily video of the current day, try creating a custom video instead") % {'currentDay': self.videoClass.getScriptStartTime().strftime("%Y%m%d")} )
                elif self.doesVideoFileExists(scanPictureDay[:8]) == True and self.videoType == "video" and VideoTag == False:
                    self.log.info("videoUtils.copyFilesToVideoDirectory(): " + _("Error, video file exists for the date: %(cfgdispday)s") % {'cfgdispday': scanPictureDay[:8]} )
                elif extensionsCount['.jpg'] < 10:
                    self.log.info("videoUtils.copyFilesToVideoDirectory(): " + _("Error, not enough jpg files for date: %(cfgdispday)s (only %(nbjpgfiles)s files)") % {'cfgdispday': scanPictureDay[:8], 'nbjpgfiles': extensionsCount['.jpg']} )
                else:
                        ...
                        if os.path.splitext(scanPictureFile)[0].isdigit() == True and int(os.path.splitext(scanPictureFile)[0]) > int(self.videoClass.getCustomVideoStart()) and int(os.path.splitext(scanPictureFile)[0]) < int(self.videoClass.getCustomVideoEnd()):
                            ...
                            if (self.videoType == "videocustom" or self.videoType == "videopost") and (keepstart != 0 or keepend != 0):
                                ...
                                if currentfilestamp >= keepstart and currentfilestamp <= keepend:
                                    if self.configSourceVideo.getConfig("cfgfilteractivate") == "yes": # Activate filter to diff files
                                        copyFile = self.compareImages(self.dirCurrentSourcePictures + scanPictureDay + "/" + scanPictureFile)
                                    else:
                                        copyFile = True
                            else:
                                if self.configSourceVideo.getConfig("cfgfilteractivate") == "yes": # Activate filter to diff files
                            ...
                            if self.videoType == "videocustom" or self.videoType == "videopost":
                                if self.configSourceVideo.getConfig("cfgvidminintervalvalue") != "0": # check time between two pictures
                                    ...
                                    SecondsBetweenPictures = self.fileUtils.SecondsBetweenPictures(TargetVideoDir, scanPictureFile)
                                    ...
                                    if SecondsBetweenPictures != None: 
                                        if self.configSourceVideo.getConfig("cfgvidmininterval") == "minutes":
                                            ...
                                            if SecondsBetweenPictures < ReferenceInterval:
                                                copyFile = False				
                                        if copyFile == False:
                                            self.log.info("videoUtils.run(): " + _("Video: %(VideoType)s: Discarding picture, minimum interval:  %(ReferenceInterval)s s - Current Difference: %(SecondsBetweenPictures)s, Current file: %(scanPictureFile)s ") % {'VideoType': self.videoType, 'ReferenceInterval': str(ReferenceInterval), 'SecondsBetweenPictures': str(SecondsBetweenPictures), 'scanPictureFile': str(scanPictureFile) } )	

                            if copyFile == True: # Copy files to temporary directory to be processed for video creation
                                filesCopied = filesCopied + 1
                                shutil.copy(self.dirCurrentSourcePictures + scanPictureDay + "/" + scanPictureFile, self.videoClass.getProcessVideoDir() + scanPictureFile)
                                self.log.info("videoUtils.copyFilesToVideoDirectory(): " + _("Copy picture to temporary directory: %(scanPictureFile)s") % {'VideoType': self.videoType, 'scanPictureFile': str(scanPictureFile) } )	
        return filesCopied
    ...
    def modifyPictures(self, filePath):
        ...
        if self.configSourceVideo.getConfig('cfgvideoeffect') == "sketch":
            self.log.info("videoUtils.modifyPictures(): " + _("Adding sketch effect to: %(filePath)s ") % {'filePath': filePath} )
            ...
            self.log.info("videoUtils.modifyPictures(): " + _("Adding tiltshift effect to: %(filePath)s ") % {'filePath': filePath} )
            self.pictureTransformations.TiltShift()
        elif self.configSourceVideo.getConfig('cfgvideoeffect') == "charcoal":
            self.log.info("videoUtils.modifyPictures(): " + _("Adding charcoal effect to: %(filePath)s ") % {'filePath': filePath} )
            self.pictureTransformations.Charcoal()
        elif self.configSourceVideo.getConfig('cfgvideoeffect') == "colorin":
            self.log.info("videoUtils.modifyPictures(): " + _("Adding colorin effect to: %(filePath)s ") % {'filePath': filePath} )
        ...
        if self.configSourceVideo.getConfig('cfgwatermarkactivate') == "yes":
            ...
            if watermarkFile!= None:
                self.pictureTransformations.setFilesourcePath(currentFile)
                self.pictureTransformations.setFiledestinationPath(self.videoClass.getProcessVideoDir() + "filterB.jpg")                 
                self.pictureTransformations.Watermark(self.configSourceVideo.getConfig('cfgwatermarkpositionx'), self.configSourceVideo.getConfig('cfgwatermarkpositiony'), self.configSourceVideo.getConfig('cfgwatermarkdissolve'), watermarkFile)	            
        if self.configSourceVideo.getConfig('cfgvideopreimagemagicktxt') == "yes":	
                ...
                self.pictureTransformations.Text(self.configSourceVideo.getConfig('cfgvideopreimgtextfont'), self.configSourceVideo.getConfig('cfgvideopreimgtextsize'), self.configSourceVideo.getConfig('cfgvideopreimgtextgravity'), self.configSourceVideo.getConfig('cfgvideopreimgtextbasecolor'), self.configSourceVideo.getConfig('cfgvideopreimgtextbaseposition'), self.configSourceVideo.getConfig('cfgvideopreimgtext'), self.formatDateLegend(pictureTime, self.configSourceVideo.getConfig('cfgvideopreimgdateformat')), self.configSourceVideo.getConfig('cfgvideopreimgtextovercolor'), self.configSourceVideo.getConfig('cfgvideopreimgtextoverposition'))                
        if self.configSourceVideo.getConfig('cfgvideopreresize') != "no":
                self.log.info("videoUtils.modifyPictures(): " + _("Video: ImageMagick: Resizing: %(File)s to %(Size)s") % {'File': filePath, 'Size': self.configSourceVideo.getConfig('cfgvideopreresizeres')} )
    ...
    def modifyPicturesPost(self, filePath):
        ...
        if self.configSourceVideo.getConfig('cfgrotateactivate') == "yes":
            self.log.info("videoUtils.modifyPicturesPost(): " + _("Rotating file: %(filePath)s by %(degrees)s degrees") % {'filePath': filePath, 'degrees': self.configSourceVideo.getConfig('cfgrotateangle')} )
            self.pictureTransformations.rotate(self.configSourceVideo.getConfig('cfgrotateangle'))
        
        if self.configSourceVideo.getConfig('cfgthumbnailactivate') == "yes" and self.configSourceVideo.getConfig('cfgtransitionactivate') != "yes":	
            self.log.info("videoUtils.modifyPicturesPost(): " + _("Creating thumbnail of file: %(filePath)s") % {'filePath': filePath} )
            self.pictureTransformations.setFiledestinationPath(self.videoClass.getProcessVideoDir() + "thumbnail.jpg")
            self.pictureTransformations.crop(self.configSourceVideo.getConfig('cfgthumbnailsrccropsizewidth'), self.configSourceVideo.getConfig('cfgthumbnailsrccropsizeheight'), self.configSourceVideo.getConfig('cfgthumbnailsrccropxpos'), self.configSourceVideo.getConfig('cfgthumbnailsrccropypos'))
            self.pictureTransformations.setFilesourcePath(self.videoClass.getProcessVideoDir() + "thumbnail.jpg")
            self.pictureTransformations.resize(self.configSourceVideo.getConfig('cfgthumbnaildstsizewidth') + "x" + self.configSourceVideo.getConfig('cfgthumbnaildstsizeheight'))
            if self.configSourceVideo.getConfig('cfgthumbnailborder') == "yes":	
                self.pictureTransformations.Border("#909090", "5", "5")
            self.log.info("videoUtils.modifyPicturesPost(): " + _("Thumbnail stored in: %(filePath)s") % {'filePath': self.videoClass.getProcessVideoDir() + "thumbnail.jpg"} )
            
            self.pictureTransformations.setFilesourcePath(filePath)
            self.pictureTransformations.setFiledestinationPath(filePath) 
                
        if self.configSourceVideo.getConfig('cfgtransitionactivate') != "yes" and self.configSourceVideo.getConfig('cfgcropactivate') == "yes":	
            self.log.info("videoUtils.modifyPicturesPost(): " + _("Cropping file: %(filePath)s ") % {'filePath': filePath} )
            self.pictureTransformations.crop(self.configSourceVideo.getConfig('cfgcropsizewidth'), self.configSourceVideo.getConfig('cfgcropsizeheight'), self.configSourceVideo.getConfig('cfgcropxpos'), self.configSourceVideo.getConfig('cfgcropypos'))
                        
        if self.configSourceVideo.getConfig('cfgvideosizeactivate') == "yes":	
            self.log.info("videoUtils.modifyPicturesPost(): " + _("Resizing file: %(filePath)s to: %(Size)s") % {'filePath': filePath, 'Size': self.configSourceVideo.getConfig('cfgvideopreresizeres')} )
            self.pictureTransformations.resize(self.configSourceVideo.getConfig('cfgvideosizewidth') + "x" + self.configSourceVideo.getConfig('cfgvideosizeheight'))
                    
        if self.configSourceVideo.getConfig('cfgvideoeffect') == "sketch":
            self.log.info("videoUtils.modifyPicturesPost(): " + _("Adding sketch effect to: %(filePath)s ") % {'filePath': filePath} )
            self.pictureTransformations.Sketch(self.videoClass.getProcessVideoDir())
        elif self.configSourceVideo.getConfig('cfgvideoeffect') == "tiltshift":
            self.log.info("videoUtils.modifyPicturesPost(): " + _("Adding tiltshift effect to: %(filePath)s ") % {'filePath': filePath} )
            self.pictureTransformations.TiltShift()
        elif self.configSourceVideo.getConfig('cfgvideoeffect') == "charcoal":
            self.log.info("videoUtils.modifyPicturesPost(): " + _("Adding charcoal effect to: %(filePath)s ") % {'filePath': filePath} )
            self.pictureTransformations.Charcoal()
        elif self.configSourceVideo.getConfig('cfgvideoeffect') == "colorin":
            self.log.info("videoUtils.modifyPicturesPost(): " + _("Adding colorin effect to: %(filePath)s ") % {'filePath': filePath} )
            self.pictureTransformations.ColorIn()

        if self.configSourceVideo.getConfig('cfgthumbnailactivate') == "yes":
            self.log.info("videoUtils.modifyPicturesPost(): " + _("Inserting thumbnail into: %(filePath)s") % {'filePath': filePath} )
            self.pictureTransformations.Watermark(self.configSourceVideo.getConfig('cfgwatermarkpositionx'), self.configSourceVideo.getConfig('cfgwatermarkpositiony'), self.configSourceVideo.getConfig('cfgwatermarkdissolve'), self.videoClass.getProcessVideoDir() + "thumbnail.jpg")	                        
            self.log.info("videoUtils.modifyPicturesPost(): " + _("Insertion completed, deleting thumbnail stored at: %(filePath)s") % {'filePath': self.videoClass.getProcessVideoDir() + "thumbnail.jpg"} )            
            os.remove(self.videoClass.getProcessVideoDir() + "thumbnail.jpg")	

        if self.configSourceVideo.getConfig('cfgwatermarkactivate') == "yes":
            self.log.info("videoUtils.modifyPicturesPost(): " + _("Adding Watermark to: %(File)s ") % {'File': filePath} )
            watermarkFile = None
            if os.path.isfile(self.dirCurrentSourceWatermarkDir + self.configSourceVideo.getConfig('cfgfilterwatermarkfile')):
                watermarkFile = self.dirCurrentSourceWatermarkDir + self.configSourceVideo.getConfig('cfgfilterwatermarkfile')
            elif os.path.isfile(self.dirWatermark + self.configSourceVideo.getConfig('cfgfilterwatermarkfile')):
    ...
    def transitionPictures(self):
        ...
        for scanPictureFile in sorted(os.listdir(self.videoClass.getProcessVideoDir())):
            ...
            if self.configSourceVideo.getConfig('cfgthumbnailactivate') == "yes":	
                self.log.info("videoUtils.transitionPictures(): " + _("Creating thumbnail of file: %(filePath)s") % {'filePath': filePath} )
                self.pictureTransformations.setFiledestinationPath(self.videoClass.getProcessVideoDir() + "thumbnail.jpg")
                self.pictureTransformations.crop(self.configSourceVideo.getConfig('cfgthumbnailsrccropsizewidth'), self.configSourceVideo.getConfig('cfgthumbnailsrccropsizeheight'), self.configSourceVideo.getConfig('cfgthumbnailsrccropxpos'), self.configSourceVideo.getConfig('cfgthumbnailsrccropypos'))
                self.pictureTransformations.setFilesourcePath(self.videoClass.getProcessVideoDir() + "thumbnail.jpg")
                self.pictureTransformations.resize(self.configSourceVideo.getConfig('cfgthumbnaildstsizewidth') + "x" + self.configSourceVideo.getConfig('cfgthumbnaildstsizeheight'))
                if self.configSourceVideo.getConfig('cfgthumbnailborder') == "yes":	
                    self.pictureTransformations.Border("#909090", "5", "5")
                self.log.info("videoUtils.transitionPictures(): " + _("Thumbnail stored in: %(filePath)s") % {'filePath': self.videoClass.getProcessVideoDir() + "thumbnail.jpg"} )

            self.pictureTransformations.setFilesourcePath(self.videoClass.getProcessVideoDir() + scanPictureFile)
            self.pictureTransformations.setFiledestinationPath(self.videoClass.getProcessVideoDir() + scanPictureFile) 
            
            self.log.info("videoUtils.transitionPictures(): " + _("Processing picture: %(File)s W:%(UpdatedCropWidth)s H:%(UpdatedCropHeight)s X:%(UpdatedCropX)s Y:%(UpdatedCropY)s") % {'File': self.videoClass.getProcessVideoDir() + scanPictureFile, 'UpdatedCropWidth': str(int(UpdatedCropWidth)), 'UpdatedCropHeight': str(int(UpdatedCropHeight)), 'UpdatedCropX': str(int(UpdatedCropX)), 'UpdatedCropY': str(int(UpdatedCropY))} )
            self.pictureTransformations.crop(self.configSourceVideo.getConfig('cfgthumbnailsrccropsizewidth'), self.configSourceVideo.getConfig('cfgthumbnailsrccropsizeheight'), self.configSourceVideo.getConfig('cfgthumbnailsrccropxpos'), self.configSourceVideo.getConfig('cfgthumbnailsrccropypos'))
            ...
            if self.configSourceVideo.getConfig("cfgmovefilestosource") != "no":
                ...
                if os.path.exists(self.sourceDestinationDirectory + scanPictureFile):
                    self.log.info("videoUtils.transitionPictures(): " + _("Error: File already exists %(DestinationFile)s") % {'DestinationFile': self.sourceDestinationDirectory + scanPictureFile} )
                else:
                    self.log.info("videoUtils.transitionPictures(): " + _("Copy file to: %(DestinationFile)s") % {'DestinationFile': self.sourceDestinationDirectory + scanPictureFile} )
                    shutil.copy(self.videoClass.getProcessVideoDir() + scanPictureFile, self.sourceDestinationDirectory + scanPictureFile)
                    os.remove(self.videoClass.getProcessVideoDir() + scanPictureFile)                        

    def encodeVideo(self, VideoSourceFiles, VideoDestinationFile, VideoLogFile, VideoFPS, VideoCropScaleOptions, VideoPass, VideoBitrate):        
        ...
        self.log.info("videoUtils.transitionPictures(): " + _("Mencoder: Video compression, pass: %(Pass)s") % {'Pass': VideoPass} )
        self.log.info("videoUtils.transitionPictures(): " + _("Mencoder: Source: %(VideoSourceFiles)s") % {'VideoSourceFiles': VideoSourceFiles} )
        self.log.info("videoUtils.transitionPictures(): " + _("Mencoder: Destination: %(VideoDestinationFile)s") % {'VideoDestinationFile': VideoDestinationFile} )
    ...
    def addAudio(self, AudioDir, AudioFile, TargetVideoDir, TargetVideoFilename):
        ...
        if AudioFile == "playlist.m3u":
            ...
            if JoinMP3Files != "":
                ...
                self.log.info("videoUtils.addAudio(): " + _("Building mp3 file from playlist using command: %(Command)s") % {'Command': Command} )                
                args = shlex.split(Command)
                p = subprocess.Popen(args,stdout=subprocess.PIPE,stderr=subprocess.PIPE)
                output, errors = p.communicate()
                self.log.info(output)
        ...
        self.log.info("videoUtils.addAudio(): " + _("Adding audio file using Command: %(Command)s") % {'Command': Command} )
        args = shlex.split(Command)
        p = subprocess.Popen(args,stdout=subprocess.PIPE,stderr=subprocess.PIPE)
        output, errors = p.communicate()		
    ...
    def createMP4(self, TargetVideoDir, TargetVideoFilename):
        ...
        Returns:
            None
        """  
        self.log.info("videoUtils.createMP4(): " + _("Video: Flash: Creation of the MP4 video file: %(FlashFile)s") % {'FlashFile': TargetVideoDir + TargetVideoFilename + ".mp4"} )
        ...
        self.log.info("videoUtils.createMP4(): " + _("Creating MP4 file using Command: %(Command)s") % {'Command': Command} )
        ...
        self.log.info("videoUtils.createMP4(): " + _("Video: Flash: Inserting markers on MP4 video file: %(FlashFile)s") % {'FlashFile': TargetVideoDir + TargetVideoFilename + ".mp4"} )
        Command = "MP4Box -inter 500 " + TargetVideoDir + TargetVideoFilename + ".mp4 "
        self.log.info("videoUtils.createMP4(): " + _("Creating MP4 markers using Command: %(Command)s") % {'Command': Command} )        
        args = shlex.split(Command)
        p = subprocess.Popen(args,stdout=subprocess.PIPE,stderr=subprocess.PIPE)
        output, errors = p.communicate()
        self.log.info(output)
        ...
        if os.path.isfile(TargetVideoDir + TargetVideoFilename + ".mp4"):
            if os.path.getsize(TargetVideoDir + TargetVideoFilename + ".mp4") > 500000:
                self.log.info("videoUtils.createMP4(): " + _("Proper file size: %(FlashFile)s") % {'FlashFile': TargetVideoDir + TargetVideoFilename + ".mp4"} )
                os.remove(TargetVideoDir + TargetVideoFilename + ".mp4.bak")
        else :
            self.log.info("videoUtils.createMP4(): " + _("Video: Flash: Invalid file size: %(FlashFile)s") % {'FlashFile': TargetVideoDir + TargetVideoFilename + ".mp4"} )
            shutil.move(TargetVideoDir + TargetVideoFilename + ".mp4.bak", TargetVideoDir + TargetVideoFilename + ".mp4")

        self.log.info("videoUtils.createMP4(): " + _("Video: Flash: Creation of a preview pictures, located 2 seconds from the beginning of the video"))
        ...
        self.log.info("videoUtils.createMP4(): " + _("Creating preview pictures using Command: %(Command)s") % {'Command': Command} )                
        args = shlex.split(Command)
        p = subprocess.Popen(args,stdout=subprocess.PIPE,stderr=subprocess.PIPE)
        output, errors = p.communicate()
        self.log.info(output)
        ...
        if os.path.getsize(TargetVideoDir + TargetVideoFilename + ".jpg") > 20000:		
            ...
            self.log.info("videoUtils.createMP4(): " + _("Video: Flash: Preview failed to be created, trying to create a new one located 2 seconds from the beginning of the video"))
            ...
            self.log.info("videoUtils.createMP4(): " + _("Re-trying to create preview pictures, 2 seconds into the video using Command: %(Command)s") % {'Command': Command} )                
            args = shlex.split(Command)
            p = subprocess.Popen(args,stdout=subprocess.PIPE,stderr=subprocess.PIPE)
            output, errors = p.communicate()
            self.log.info(output)
    ...
    def createVideos(self, videoformats):
        ...
        self.log.info("videoUtils.createVideos(): " + _("Processing format: %(videoformats)s - preparing parameters ") % {'videoformats': videoformats } )
        CropOptions=""
        ScaleOptions=""
        CropScaleOptions=""
        if self.configSourceVideo.getConfig("cfgvideocodecH264" + videoformats + "width") != "" and self.configSourceVideo.getConfig("cfgvideocodecH264" + videoformats + "height") == "":
            ScaleOptions = "scale=" + self.configSourceVideo.getConfig("cfgvideocodecH264" + videoformats + "width") + ":-3"
        elif self.configSourceVideo.getConfig("cfgvideocodecH264" + videoformats + "width") == "" and self.configSourceVideo.getConfig("cfgvideocodecH264" + videoformats + "height") != "":
            ScaleOptions = "scale=-3:" + self.configSourceVideo.getConfig("cfgvideocodecH264" + videoformats + "height")
        elif self.configSourceVideo.getConfig("cfgvideocodecH264" + videoformats + "width") != "" and self.configSourceVideo.getConfig("cfgvideocodecH264" + videoformats + "height") != "":
            ScaleOptions = "scale=" + self.configSourceVideo.getConfig("cfgvideocodecH264" + videoformats + "width") + ":" + self.configSourceVideo.getConfig("cfgvideocodecH264" + videoformats + "height")
        if self.configSourceVideo.getConfig("cfgvideocodecH264" + videoformats + "cropwidth") != "" and self.configSourceVideo.getConfig("cfgvideocodecH264" + videoformats + "cropheight") != "":
            CropOptions = "crop=" + self.configSourceVideo.getConfig("cfgvideocodecH264" + videoformats + "cropwidth") + ":" + self.configSourceVideo.getConfig("cfgvideocodecH264" + videoformats + "cropheight") + ":" + self.configSourceVideo.getConfig("cfgvideocodecH264" + videoformats + "cropx") + ":" + self.configSourceVideo.getConfig("cfgvideocodecH264" + videoformats + "cropy")
        ...
        self.log.info("videoUtils.createVideos(): " + _("Video: %(VideoType)s: Format: %(videoformats)s - Compression of the first pass with mencoder") % {'VideoType': self.videoType, 'videoformats': videoformats} )
        self.encodeVideo(self.videoClass.getprocessVideoFiles(), self.videoClass.getProcessVideoDir() + TargetVideoFilename, self.videoClass.getProcessVideoDir() + "currentvid.log", self.configSourceVideo.getConfig("cfgvideocodecH264" + videoformats + "fps"), CropScaleOptions, "1", self.configSourceVideo.getConfig("cfgvideocodecH264" + videoformats + "bitrate"))
        if self.configSourceVideo.getConfig("cfgvideocodecH264" + videoformats + "2pass") == "yes":
            self.log.info("videoUtils.createVideos(): " + _("Video: %(VideoType)s: Format: %(videoformats)s - Compression of the second pass with mencoder") % {'VideoType': self.videoType, 'videoformats': videoformats} )
            self.encodeVideo(self.videoClass.getprocessVideoFiles(), self.videoClass.getProcessVideoDir() + TargetVideoFilename, self.videoClass.getProcessVideoDir() + "currentvid.log", self.configSourceVideo.getConfig("cfgvideocodecH264" + videoformats + "fps"), CropScaleOptions, "2", self.configSourceVideo.getConfig("cfgvideocodecH264" + videoformats + "bitrate"))

        if os.path.getsize(self.videoClass.getProcessVideoDir() + TargetVideoFilename) > 50000:
            self.log.info("videoUtils.createVideos(): " + _("Mencoder: Video creation completed: %(VideoFile)s") % {'VideoFile': self.videoClass.getProcessVideoDir() + TargetVideoFilename } )
            if self.configSourceVideo.getConfig("cfgvideoaddaudio") == "yes" and self.configSourceVideo.getConfig("cfgvideoaudiofile") != "":
                if os.path.isfile(self.Cfgselfaudiodir + self.configSourceVideo.getConfig("cfgvideoaudiofile")):
                    self.addAudio(self.Cfgselfaudiodir, self.configSourceVideo.getConfig("cfgvideoaudiofile"), self.videoClass.getProcessVideoDir(), TargetVideoFilename)
                elif os.path.isfile(self.Cfgaudiodir + self.configSourceVideo.getConfig("cfgvideoaudiofile")):
                    self.addAudio(self.Cfgaudiodir, self.configSourceVideo.getConfig("cfgvideoaudiofile"), self.videoClass.getProcessVideoDir(), TargetVideoFilename)
            ...
            if os.path.isfile(self.videoClass.getProcessVideoDir() + TargetVideoFilename):
                self.log.info("videoUtils.createVideos(): " + _("Video: %(VideoType)s: Copy of the video file into archives") % {'VideoType': self.videoType} )
                shutil.copy(self.videoClass.getProcessVideoDir() + TargetVideoFilename, self.dirCurrentSourceVideos + TargetVideoFilename)
                self.log.info("videoUtils.createVideos(): " + _("Video: %(VideoType)s: Copy of the video file into live directory") % {'VideoType': self.videoType} )
                shutil.copy(self.videoClass.getProcessVideoDir() + TargetVideoFilename, self.dirCurrentSourceLive + TargetLiveVideoFilename)
            ...
            if os.path.isfile(self.videoClass.getProcessVideoDir() + TargetVideoFilename + ".mp4"):
                self.log.info("videoUtils.createVideos(): " + _("Video: %(VideoType)s: Copy of the MP4 video file into archives") % {'VideoType': self.videoType} )
                shutil.copy(self.videoClass.getProcessVideoDir() + TargetVideoFilename + ".mp4", self.dirCurrentSourceVideos + TargetVideoFilename + ".mp4")
                self.log.info("videoUtils.createVideos(): " + _("Video: %(VideoType)s: Copy of the MP4 video file into live directory") % {'VideoType': self.videoType} )
                shutil.copy(self.videoClass.getProcessVideoDir() + TargetVideoFilename + ".mp4", self.dirCurrentSourceLive + TargetLiveVideoFilename + ".mp4")
            ...
            if os.path.isfile(self.videoClass.getProcessVideoDir() + TargetVideoFilename + ".jpg"):
                self.log.info("videoUtils.createVideos(): " + _("Video: %(VideoType)s: Copy of the JPG preview file into archives") % {'VideoType': self.videoType} )
                shutil.copy(self.videoClass.getProcessVideoDir() + TargetVideoFilename + ".jpg", self.dirCurrentSourceVideos + TargetVideoFilename + ".jpg")
                self.log.info("videoUtils.createVideos(): " + _("Video: %(VideoType)s: Copy of the JPG preview file into live directory") % {'VideoType': self.videoType} )
                shutil.copy(self.videoClass.getProcessVideoDir() + TargetVideoFilename + ".jpg", self.dirCurrentSourceLive + TargetLiveVideoFilename + ".jpg")
                os.remove(self.videoClass.getProcessVideoDir() + TargetVideoFilename + ".jpg")


            return TargetVideoFilename
        else:
            self.log.info("video.run(): " + _("Video: %(VideoType)s: Error while creating video: %(VideoFile)s") % {'VideoType': self.videoType, 'VideoFile': self.videoClass.getProcessVideoDir() + TargetVideoFilename} )       
            return False
    ...
    def sendVideos(self, TargetVideoFilename, videoformats):
        ...
        if self.configSourceVideo.getConfig("cfgftpmainserveraviid") != "":
            self.log.info("videoUtils.sendVideos(): " + _("Video: %(VideoType)s: Sending file via FTP: %(FTPFile)s") % {'VideoType': self.videoType, 'FTPFile': self.dirCurrentSourceVideos + TargetVideoFilename} )
            self.transferUtils.transferFile(self.videoClass.getScriptStartTime(), self.dirCurrentSourceVideos + TargetVideoFilename, TargetVideoFilename, self.configSourceVideo.getConfig('cfgftpmainserveraviid'), self.configSourceVideo.getConfig('cfgftpmainserveraviretry'))
        if self.configSourceVideo.getConfig("cfgftpmainservermp4id") != "":
            self.log.info("videoUtils.sendVideos(): " + _("Video: %(VideoType)s: Sending file via FTP: %(FTPFile)s") % {'VideoType': self.videoType, 'FTPFile': self.dirCurrentSourceVideos + TargetVideoFilename + ".mp4"} )
            self.transferUtils.transferFile(self.videoClass.getScriptStartTime(), self.dirCurrentSourceVideos + TargetVideoFilename + ".mp4", TargetVideoFilename + ".mp4", self.configSourceVideo.getConfig('cfgftpmainservermp4id'), self.configSourceVideo.getConfig('cfgftpmainservermp4retry'))
            self.log.info("videoUtils.sendVideos(): " + _("Video: %(VideoType)s: Sending file via FTP: %(FTPFile)s") % {'VideoType': self.videoType, 'FTPFile': self.dirCurrentSourceVideos + TargetVideoFilename + ".jpg"} )
            self.transferUtils.transferFile(self.videoClass.getScriptStartTime(), self.dirCurrentSourceVideos + TargetVideoFilename + ".jpg", TargetVideoFilename + ".jpg", self.configSourceVideo.getConfig('cfgftpmainservermp4id'), self.configSourceVideo.getConfig('cfgftpmainservermp4retry'))

        if self.configSourceVideo.getConfig("cfgftphotlinkserveraviid") != "":
            self.log.info("videoUtils.sendVideos(): " + _("Video: %(VideoType)s: Sending file via FTP: %(FTPFile)s") % {'VideoType': self.videoType, 'FTPFile': self.dirCurrentSourceVideos + TargetVideoFilename} )
            self.transferUtils.transferFile(self.videoClass.getScriptStartTime(), self.dirCurrentSourceLive + TargetLiveVideoFilename, TargetLiveVideoFilename, self.configSourceVideo.getConfig('cfgftphotlinkserveraviid'), self.configSourceVideo.getConfig('cfgftphotlinkserveraviretry'))                            
        if self.configSourceVideo.getConfig("cfgftphotlinkservermp4id") != "":
            self.log.info("videoUtils.sendVideos(): " + _("Video: %(VideoType)s: Sending file via FTP: %(FTPFile)s") % {'VideoType': self.videoType, 'FTPFile': self.dirCurrentSourceVideos + TargetVideoFilename + ".mp4"} )
            self.transferUtils.transferFile(self.videoClass.getScriptStartTime(), self.dirCurrentSourceLive + TargetLiveVideoFilename + ".mp4", TargetLiveVideoFilename + ".mp4", self.configSourceVideo.getConfig('VideoType'), self.configSourceVideo.getConfig('cfgftphotlinkservermp4retry'))                        
            self.log.info("videoUtilss.sendVideos(): " + _("Video: %(VideoType)s: Sending file via FTP: %(FTPFile)s") % {'VideoType': self.videoType, 'FTPFile': self.dirCurrentSourceVideos + TargetVideoFilename + ".jpg"} )
            self.transferUtils.transferFile(self.videoClass.getScriptStartTime(), self.dirCurrentSourceLive + TargetLiveVideoFilename + ".jpg", TargetLiveVideoFilename + ".jpg", self.configSourceVideo.getConfig('VideoType'), self.configSourceVideo.getConfig('cfgftphotlinkservermp4retry'))                        
