...
class pictureTransformations(object):
        ...
        if pictureSize != "":
            if os.path.isfile(self.dirImageMagick + "convert"):
                ...
                subprocess.check_call(
                    [self.dirImageMagick + "convert", self.getFilesourcePath(), "-scale", pictureSize + '!',
                     self.getFiledestinationPath()])
                endTimer = timer()
                self.log.info(
                    "pictureTransformations.resize(): " + _("Resized picture to %(pictureSize)s in %(timer)s ms") % {
                        'pictureSize': pictureSize, 'timer': int((endTimer - startTimer) * 1000)})
            else:
    ...
    def crop(self, Width, Height, XPos, YPos):
        ...
        if os.path.isfile(self.dirImageMagick + "convert"):
            startTimer = timer()
            convert = subprocess.check_call([self.dirImageMagick + "convert", self.getFilesourcePath(), "-crop",
                                             Width + 'x' + Height + '+' + XPos + '+' + YPos + '!',
                                             self.getFiledestinationPath()])
            endTimer = timer()
            self.log.info("pictureTransformations.crop(): " + _(
                "Cropped zone size: %(Width)sx%(Height)s Position: x: %(XPos)s y: %(YPos)s in %(timer)s ms") % {
    ...
    def rotate(self, rotateAngle):
        ...
        if os.path.isfile(self.dirImageMagick + "convert"):
            startTimer = timer()
            subprocess.check_call([self.dirImageMagick + "convert", self.getFilesourcePath(), "-rotate", rotateAngle,
                                   self.getFiledestinationPath()])
            endTimer = timer()
            self.log.info("pictureTransformations.rotate(): " + _(
                "Rotated picture by %(rotateAngle)s degrees in %(timer)s ms") % {'rotateAngle': rotateAngle,
                                                                                 'timer': int(
    ...
    def Border(self, Color, HBorder, VBorder, NewPath):
        ...
        if os.path.isfile(self.dirImageMagick + "convert"):
            ...
            self.log.info(
                "pictureTransformations.Border(): " + _("Color: %(Color)s Width: %(BorderWidth)s") % {'Color': Color,
                                                                                                      'BorderWidth': HBorder + ":" + VBorder})
            convert = subprocess.check_call(
                [self.dirImageMagick + "convert", self.getFilesourcePath(), "-bordercolor", Color, "-border",
                 HBorder + 'x' + VBorder, NewPath])
        else:
            self.log.debug(_("Error: Convert binary (ImageMagick) not found"))
            sys.exit()
    ...
    def Watermark(self, XPos, YPos, Dissolve, Watermarkfile):
        ...
        if os.path.isfile(self.dirImageMagick + "composite"):
            ...
            composite = subprocess.check_call(
                [self.dirImageMagick + "composite", "-dissolve", Dissolve + '%', "-geometry", '+' + XPos + '+' + YPos,
                 Watermarkfile, self.getFilesourcePath(), self.getFiledestinationPath()])
            endTimer = timer()
            self.log.info("pictureTransformations.Watermark(): " + _(
                "Added watermark file %(Watermarkfile)s at position: x: %(XPos)s y: %(YPos)s Transparency: %(Dissolve)s percent in %(timer)s ms") % {
                              'Watermarkfile': Watermarkfile, 'XPos': XPos, 'YPos': YPos, 'Dissolve': Dissolve,
                              'timer': int((endTimer - startTimer) * 1000)})
        else:
            self.log.debug(
                "pictureTransformations.Watermark(): " + _("Error: Composite binary (ImageMagick) not found"))
            sys.exit()

    def Text(self, Cfgimgtextfont, Cfgimgtextsize, Cfgimgtextgravity, Cfgimgtextbasecolor, Cfgimgtextbaseposition,
             Cfgimgtext, Cfgdisplaydate, Cfgimgtextovercolor, Cfgimgtextoverposition):
        ...
        if os.path.isfile(self.dirImageMagick + "convert"):
            ...
            mogrify = subprocess.check_call(
                [self.dirImageMagick + "convert", "-font", Cfgimgtextfont, "-pointsize", Cfgimgtextsize, "-draw",
                 "gravity " + Cfgimgtextgravity + " fill " + Cfgimgtextbasecolor + " text " + Cfgimgtextbaseposition + " '" + Cfgimgtext + Cfgdisplaydate + "' fill " + Cfgimgtextovercolor + " text " + Cfgimgtextoverposition + " '" + Cfgimgtext + Cfgdisplaydate + "' ",
                 self.getFilesourcePath(), self.getFiledestinationPath()])
            endTimer = timer()
            self.log.info("pictureTransformations.Text(): " + _(
                "Added text to the picture: %(Text)s Font: %(Cfgimgtextfont)s Gravity: %(Cfgimgtextgravity)s in %(timer)s ms") % {
                              'Text': Cfgimgtext + Cfgdisplaydate, 'Cfgimgtextfont': Cfgimgtextfont,
                              'Cfgimgtextgravity': Cfgimgtextgravity, 'timer': int((endTimer - startTimer) * 1000)})
        else:
    ...
    def Sketch(self, TargetDir):
        ...
        if os.path.isfile(TargetDir + "pencil_tile.gif") == False:
            ...
            self.log.info(
                "pictureTransformations.Sketch(): " + _("Running command to generate pencil tile: %(Command)s") % {
                    'Command': Command})
        ...
        self.log.info("pictureTransformations.Sketch(): " + _("Running command to creaste sketch: %(Command)s") % {
            'Command': Command})
        args = shlex.split(Command)
        p = subprocess.Popen(args, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
        p.communicate()
    ...
    def TiltShift(self):
        ...
        self.log.info(
            "pictureTransformations.TiltShift(): " + _("Running command to create tiltshift effect: %(Command)s") % {
                'Command': Command})
        args = shlex.split(Command)
        p = subprocess.Popen(args, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
    ...
    def Charcoal(self):
        ...
        self.log.info(
            "pictureTransformations.Charcoal(): " + _("Running command to create charcoal effect: %(Command)s") % {
                'Command': Command})
        args = shlex.split(Command)
        p = subprocess.Popen(args, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
        output, errors = p.communicate()
    ...
    def ColorIn(self):
        ...
        self.log.info(
            "pictureTransformations.ColorIn(): " + _("Running command to create color-in effect: %(Command)s") % {
                'Command': Command})
        args = shlex.split(Command)
        p = subprocess.Popen(args, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
        output, errors = p.communicate()
    ...
    def VirtualPixel(self, PixelsSpread):
        ...
        self.log.info(_("pictureTransformations.VirtualPixel(): Applying a spread of: %(PixelsSpread)s pixels") % {
            'PixelsSpread': PixelsSpread})
        Command = self.dirImageMagick + "convert " + self.getFilesourcePath() + " -interpolate nearest -virtual-pixel mirror -spread " + str(
            PixelsSpread) + " " + self.getFiledestinationPath()
