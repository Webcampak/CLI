...
class timeUtils(object):
    ...
    def getCurrentSourceTime(self, sourceConfig):
        self.log.debug("timeUtils.getCurrentSourceTime(): Start")
        if (
            sourceConfig.getConfig("cfgcapturetimezone") != ""
        ):  # Update the timezone from UTC to the source's timezone
            self.log.info(
                "timeUtils.getCurrentSourceTime(): Source Timezone is: %(sourceTimezone)s"
        ...
        self.log.info(
            "timeUtils.getCurrentSourceTime(): Current source time: %(cfgnowsource)s"
            % {"cfgnowsource": cfgnowsource.isoformat()}
        )
        return cfgnowsource

    # Using a webcampak timestamp, capture the file date and time
    def getTimeFromFilename(self, fileName, sourceConfig, dateFormat="YYYYMMDDHHMMSS"):
        self.log.debug("timeUtils.getTimeFromFilename(): Start")
        self.log.info(
            "timeUtils.getTimeFromFilename(): Extract time from: %(fileName)s using format %(dateFormat)s"
            % {"fileName": fileName, "dateFormat": dateFormat}
        )
        try:
            if dateFormat == "YYYYMMDD_HHMMSS":
                fileTime = datetime.strptime(
                    os.path.splitext(os.path.basename(fileName))[0][0:15],
                    "%Y%m%d_%H%M%S",
                )
            else:
                fileTime = datetime.strptime(
                    os.path.splitext(os.path.basename(fileName))[0], "%Y%m%d%H%M%S"
                )
            if (
                sourceConfig.getConfig("cfgcapturetimezone") != ""
            ):  # Update the timezone from UTC to the source's timezone
                self.log.info(
                    "timeUtils.getTimeFromFilename(): Source timezone is: %(sourceTimezone)s"
                    % {"sourceTimezone": sourceConfig.getConfig("cfgcapturetimezone")}
                )
                sourceTimezone = tz.gettz(sourceConfig.getConfig("cfgcapturetimezone"))
            ...
            self.log.info(
                "timeUtils.getTimeFromFilename(): Picture date is: %(picDate)s"
    ...
    def getTimeFromExif(self, filePath, sourceConfig):
        self.log.info("timeUtils.getTimeFromExif(): Start")
        self.log.info(
            "timeUtils.getTimeFromExif(): Extract EXIF time from: %(filePath)s"
            % {"filePath": filePath}
        )
        try:
            ...
            self.log.info(
                "timeUtils.getTimeFromExif(): Failed to open %(filePath)s as a picture"
                % {"filePath": filePath}
            )
            return False
            ...
            for tag, value in list(exifinfo.items()):
                ...
                if decoded == "DateTimeDigitized":
                    ...
                    if (
                        ...
                        self.log.info(
                            "timeUtils.getTimeFromExif(): Source timezone is: %(sourceTimezone)s"
                            % {
                                "sourceTimezone": sourceConfig.getConfig(
                                    "cfgcapturetimezone"
                                )
                            }
                        )
                        sourceTimezone = tz.gettz(
                            sourceConfig.getConfig("cfgcapturetimezone")
                        )
                        fileTime = fileTime.replace(tzinfo=tz.gettz("UTC"))
                    ...
                    return fileTime
                    self.log.info(
                        "timeUtils.getTimeFromExif(): Picture date is: %(picDate)s"
                        % {"picDate": fileTime.isoformat()}
                    )
                    break
        return False
    ...
    def getTimeFromFiledate(self, filePath, sourceConfig):
        self.log.info("timeUtils.getTimeFromFiledate(): Start")
        self.log.info(
            "timeUtils.getTimeFromFiledate(): Extract time from: %(filePath)s"
            % {"filePath": filePath}
        )
        try:
            ...
            if (
                ...
                self.log.info(
                    "timeUtils.getTimeFromFiledate(): Source timezone is: %(sourceTimezone)s"
                    % {"sourceTimezone": sourceConfig.getConfig("cfgcapturetimezone")}
                )
                sourceTimezone = tz.gettz(sourceConfig.getConfig("cfgcapturetimezone"))
                fileTime = fileTime.replace(tzinfo=tz.gettz("UTC"))
                fileTime = fileTime.astimezone(sourceTimezone)
            self.log.info(
                "timeUtils.getTimeFromFiledate(): Picture date is: %(picDate)s"
                % {"picDate": fileTime.isoformat()}
            )
            return fileTime
        except:
