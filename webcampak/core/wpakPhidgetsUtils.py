...
class phidgetsUtils(object):
    ...
    def scan_ports(self):
        ...
        self.log.debug("phidgetsUtils.scan_ports(): Start")
        ...
        for current_port in range(0, 8):
            ...
            self.log.info(
                "phidgetsUtils.scan_ports(): Scanning port: %(current_port)s, RAW value: %(sensor_value)s"
                % {"current_port": str(current_port), "sensor_value": str(sensor_value)}
            )
            if sensor_value is not None:
                ...
                self.log.info(
                    "phidgetsUtils.scan_ports(): Scanning port: %(current_port)s, Temperature value: %(sensor_temperature)s"
                    % {
                        "current_port": str(current_port),
                        "sensor_temperature": str(sensor_temperature),
                    }
                )
                self.log.info(
                    "phidgetsUtils.scan_ports(): Scanning port: %(current_port)s, Luminosity value: %(sensor_luminosity)s"
                    % {
                        "current_port": str(current_port),
                        "sensor_luminosity": str(sensor_luminosity),
                    }
                )
                self.log.info(
                    "phidgetsUtils.scan_ports(): Scanning port: %(current_port)s, Pressure value: %(sensor_pressure)s"
                    % {
                        "current_port": str(current_port),
                        "sensor_pressure": str(sensor_pressure),
                    }
                )
                self.log.info(
                    "phidgetsUtils.scan_ports(): Scanning port: %(current_port)s, Humidity value: %(sensor_humidity)s"
                    % {
                        "current_port": str(current_port),
                        "sensor_humidity": str(sensor_humidity),
                    }
                )
    ...
    def email_user_restart(self, before_cameras, after_cameras):
        ...
        email_content_filepath = (
            self.dirCurrentLocaleMessages + "camera_restart_content.txt"
        )
        email_subject_filepath = (
            self.dirCurrentLocaleMessages + "camera_restart_subject.txt"
        )
        if os.path.isfile(email_content_filepath) == False:
            email_content_filepath = (
                ...
                + "camera_restart_content.txt"
            )
            email_subject_filepath = (
                self.dirLocale
                + "en_US.utf8/"
                + self.dirLocaleMessage
                + "camera_restart_subject.txt"
            )
        self.log.info(
            "phidgetsUtils.email_user_restart(): Using message subject file: %(email_subject_filepath)s"
            % {"email_subject_filepath": email_subject_filepath}
        )
        self.log.info(
            "phidgetsUtils.email_user_restart(): Using message content file: %(email_content_filepath)s"
        ...
        if (
            os.path.isfile(email_content_filepath)
            and os.path.isfile(email_subject_filepath)
            and len(email_field_to) > 0
        ):
            ...
            email_content = email_content.replace(
                "#CAMERASBEFORE#", before_cameras_email
            )
            after_cameras_email = ""
            ...
            email_subject = email_subject.replace(
                "#CURRENTSOURCE#", self.currentSourceId
            )
            ...
            self.log.debug(
                "captureEmails.sendCaptureSuccess(): Unable to find default translation files to be used"
            )

    def restartCamera(self):
        """Restart a gphoto camera based on configured ports"""
        self.log.debug("phidgetsUtils.restartCamera(): Start")
        phidget_activated = self.configGeneral.getConfig("cfgphidgetactivate")
        phidget_camera_relayport = self.configSource.getConfig(
            "cfgphidgetcamerarelayport"
        )
        if phidget_camera_relayport == "":
            phidget_camera_relayport = 0
        phidget_camera_sensorport = self.configSource.getConfig(
            "cfgphidgetcamerasensorport"
        )
        phidget_camera_pause = self.configSource.getConfig("cfgphidgetcamerapause")
        self.log.info(
            "phidgetsUtils.restartCamera(): Phidget Enabled: %(phidgetEnabled)s"
            % {"phidgetEnabled": str(phidget_activated)}
        )
        self.log.info(
            "phidgetsUtils.restartCamera(): Phidget Camera Relay Port: %(phidget_camera_relayport)s"
            % {"phidget_camera_relayport": str(phidget_camera_relayport)}
        )
        self.log.info(
            "phidgetsUtils.restartCamera(): Phidget Camera Sensor Port: %(phidget_camera_sensorport)s"
            % {"phidget_camera_sensorport": str(phidget_camera_sensorport)}
        )
        self.log.info(
            "phidgetsUtils.restartCamera(): Phidget Camera Pause: %(phidget_camera_pause)s seconds"
            % {"phidget_camera_pause": str(phidget_camera_pause)}
        )

        before_restart = Gphoto(self.log).get_cameras()
        for camera in before_restart:
            self.log.info(
                "phidgetsUtils.restartCamera(): Camera: %(camera_model)s connected to USB: %(usb_port)s"
                % {
        ...
        if phidget_activated == "yes":
            ...
            relay_state = phidgetsClass.getSensorRawValue(
                int(phidget_camera_sensorport)
            )
            self.log.info(
                "phidgetsUtils.restartCamera(): Phidget Relay Sensor value: %(relay_state)s"
                % {"relay_state": str(relay_state)}
            )

            phidgetsClass.setOutputRawValue(int(phidget_camera_relayport), True)
            self.log.info("phidgetsUtils.restartCamera(): Phidget port set to True")
            time.sleep(2)
            relay_state = phidgetsClass.getSensorRawValue(
                int(phidget_camera_sensorport)
            )
            ...
            self.log.info(
                "phidgetsUtils.restartCamera(): Pausing fot %(phidget_camera_pause)s seconds to let the camera drain all power"
                % {"phidget_camera_pause": str(phidget_camera_pause)}
            )
            time.sleep(int(phidget_camera_pause))

            phidgetsClass.setOutputRawValue(int(phidget_camera_relayport), False)
            self.log.info("phidgetsUtils.restartCamera(): Phidget port set to False")
            time.sleep(int(phidget_camera_pause))
            ...
            self.log.info("phidgetsUtils.restartCamera(): Phidgets board not enabled")
