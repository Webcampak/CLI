...
class statsRrd(object):
    ...
    def run(self):
        ...
        if (
            self.configSource.getConfig("cfgphidgetsensorsgraph") == "yes"
            and self.configSource.getConfig("cfgsourceactive") == "yes"
        ):
            self.log.info(
                "statsrrd.run(): Initiate RRD Graph creation for source: %(currentSourceId)s"
            ...
            for currentCaptureFile in allSensorsFiles:
                self.log.info(
                    "statsrrd.run(): Processing: %(currentCaptureFile)s"
                ...
                if captureInterval != None:
                    self.log.info(
                        "statsrrd.run(): Capture interval: %(captureInterval)s seconds"
                    ...
                    for currentSensor in sensors:
                        if (
                            ...
                            self.log.info(
                                "statsrrd.run(): Currently processing Sensor: %(currentSensor)s"
                            ...
                            for line in open(currentCaptureFile).readlines():
                                try:
                                    ...
                                    self.log.error(
                                        "statsrrd.run(): Unable to decode JSON line: "
                                        + line
                                    )
                                    break
                                sensorDate = dateutil.parser.parse(
                                    currentCaptureLine["date"]
                                )
                                currentTimestamp = int(
                                    time.mktime(sensorDate.timetuple())
                                )
                                ...
                                if "sensors" in currentCaptureLine:
                                    if currentCaptureLine["sensors"] != None:
                                        if (
                                            currentSensor
                                            in currentCaptureLine["sensors"]
                                        ):
                                            ValueTable[
                                                currentTimestamp
                                            ] = currentCaptureLine["sensors"][
                                                currentSensor
                                            ][
                                                "value"
                                            ]
                                            SensorLegend = currentCaptureLine[
                                                "sensors"
                                            ][currentSensor]["legend"]
                                            if (
                                                "color"
                                                in currentCaptureLine["sensors"][
                                                    currentSensor
                                                ...
                                                SensorColor = currentCaptureLine[
                                                    "sensors"
                                                ][currentSensor]["color"]
                            ...
                            self.log.info(
                                "statsrrd.run(): Preparing the RRD base file: %(SensorRRDFile)s"
                                % {
                                    "SensorRRDFile": str(
                                        self.dirCurrentSourcePictures
                                        + sensorsDay
                            ...
                            for i in range(len(ValueTableKeys)):
                                self.log.info(
                                    "statsrrd.run(): Adding Timestamp: %(currentTimestamp)s - Value: %(currentValue)s"
                                    % {
                                ...
                                if i == 0:
                                    ...
                                    currentTimestamp = (
                                        currentTimestamp + captureInterval
                                    )
                                    ret = rrdtool.update(
                                        str(
                                            self.dirCurrentSourcePictures
                                        ...
                                        str(int(currentTimestamp))
                                        + ":"
                                        + str(ValueTable[ValueTableKeys[i]]),
                                    )
                            ...
                            self.log.info(
                                "statsrrd.run(): PNG Graph created: %(pnggraph)s"
                                % {
                                    "pnggraph": str(
                                        self.dirCurrentSourcePictures
                                        + str(sensorsDay)
                            ...
                            if (
                                ...
                                currentTime = self.timeUtils.getCurrentSourceTime(
                                    self.configSource
                                )
                                self.transferUtils.transferFile(
                                    ...
                                    + currentSensor
                                    + ".rrd",
                                    "sensor-" + currentSensor + ".rrd",
                                    self.configSource.getConfig(
                                        "cfgftpphidgetserverid"
                                    ),
                                    self.configSource.getConfig(
                                        "cfgftpphidgetserverretry"
                                    ),
                                )
                                self.transferUtils.transferFile(
                                    currentTime,
            ...
            self.log.info(
                "statsrrd.run(): Creation of the RRD Graph disabled for source: %(currentSourceId)s"
                % {"currentSourceId": str(self.currentSourceId)}
            )
    ...
    def getSensorsFromFile(self, currentCaptureFile):
        ...
        """
        self.log.debug("statsrrd.getSensorsFromFile(): Start")
        sensors = {}
        for line in open(currentCaptureFile).readlines():
            ...
            try:
                ...
                self.log.error(
                    "statsrrd.getSensorsFromFile(): Unable to decode JSON line: " + line
                )
                break
            if "sensors" in currentCaptureLine:
                if currentCaptureLine["sensors"] != None:
    ...
    def getSensorDayFromFile(self, currentCaptureFile):
        ...
        """
        self.log.debug("statsrrd.getDayFromFile(): Start")
        date = None
        for line in open(currentCaptureFile).readlines():
            try:
                ...
                self.log.error(
                    "statsrrd.getSensorDayFromFile(): Unable to decode JSON line: "
                    + line
                )
                break
    ...
    def getCaptureIntervalFromFile(self, currentCaptureFile):
        ...
        """
        self.log.debug("statsrrd.getCaptureIntervalFromFile(): Start")
        date = None
        for line in open(currentCaptureFile).readlines():
            try:
                currentSensorLine = json.loads(line)
                ...
                self.log.error(
                    "statsrrd.getCaptureIntervalFromFile(): Unable to decode JSON line: "
                    + line
                )
                break
