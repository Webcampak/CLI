...
class xferClear(object):
    ...
    def run(self):
        ...
        self.log.info(
            "xferClear.run(): " + _("Removing queued jobs: %(dirXferQueue)s") % {'dirXferQueue': self.dirXferQueue})
        shutil.rmtree(self.dirXferQueue)
        self.log.info("xferClear.run(): " + _("Removing threads and jobs: %(dirXferThreads)s") % {
            'dirXferThreads': self.dirXferThreads})
        shutil.rmtree(self.dirXferThreads)
        self.log.info(
            "xferClear.run(): " + _("Removing failed jobs: %(dirXferFailed)s") % {'dirXferFailed': self.dirXferFailed})
